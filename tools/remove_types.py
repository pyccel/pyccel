...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Tool for removing unnecessary `@types` decorators"
    )

    parser.add_argument(
        "folder",
        type=str,
        help="The folder in which the @types decorator will be looked for",
    )
    ...
    for dirpath, dirnames, filenames in os.walk(args.folder):
        for name in filenames:
            ...
            if (
                ...
                or os.path.abspath(filename) == __file__
            ...
            while i < n:
                if "@types" in lines[i]:
                    ...
                    if len(current_types) == 1:
                        ...
                        quoted_types = [
                            q.strip() for q in type_annotation.findall(type_strs)
                        ]
                        ...
                        for s in type_strs.split(","):
                            ...
                            if s == "None":
                                ...
                                all_types.append(s)
                        type_annotations = [
                            s.replace("real", "float").strip() for s in all_types
                        ]
                        if len(arguments) == 1 and arguments[0] == "":
                            ...
                            arg_dict = {
                                da[0]: "" if "=" not in a else f" = {da[1]}"
                                for a, da in zip(arguments, default_args)
                            }
                            try:
                            ...
                            new_args = [
                                f"{key} : {annot}{default}"
                                for (key, default), annot in zip(
                                    arg_dict.items(), type_annotations
                                )
                            ]
                        ...
                        type_strs = [
                            t.split("(", 1)[1].rsplit(")", 1)[0] for t in current_types
                        ]
                        new_lines.append(
                            " " * indent + f"@template('T', [{', '.join(type_strs)}])\n"
                        )
                        added_template = True
                        default_args = [a.split("=") for a in arguments]
                        arg_dict = {
                        ...
                        new_args = [
                            f"{key} : 'T'{default}" for key, default in arg_dict.items()
                        ]
                        new_lines.append(f"{start}({', '.join(new_args)}){end}")
                    else:
            ...
            if uses_types:
                new_lines.insert(
                    import_types_line, "from pyccel.decorators import types\n"
                )
