...
def compile_c(path_dir, test_file, dependencies, is_mod=False):
    ...
    generated_file = insert_pyccel_folder(test_file).with_suffix(".c")
    generated_dependencies = [
        insert_pyccel_folder(Path(d)).with_suffix(".c") for d in dependencies
    ]
...
def compile_fortran(path_dir, test_file, dependencies, is_mod=False):
    ...
    generated_file = insert_pyccel_folder(test_file).with_suffix(".f90")
    generated_dependencies = [
        insert_pyccel_folder(Path(d)).with_suffix(".f90") for d in dependencies
    ]
    compile_fortran_or_c(
        compiler_info,
        path_dir,
