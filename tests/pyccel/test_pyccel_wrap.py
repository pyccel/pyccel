...
low_level_suffix = {"c": ".c", "fortran": ".f90"}
...
def check_pyccel_wrap_and_call_translation(
    ...
    l_run = subprocess.run(
        [exe_file], text=True, capture_output=True, cwd=cwd, check=True
    )
...
def test_convert_only(language, tmp_path):
    ...
    expected_file = parse_generated_module_file(p.stdout, low_level_suffix[language])
