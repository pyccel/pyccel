...
# ==============================================================================
@pytest.mark.skip(reason="do not retrieve the leading term right")
def test_complexity_ex1():
    ...
    comp = [ADD/(3 * READ + WRITE),  ## add
            ...
            ((n-1) * MUL) / ((2 * READ + WRITE) * n),  # factorial
            ...
            FLOOR(2 * READ + WRITE),  # floor_real_scalar
            ...
            (SIN+MUL+ADD+DIV)/(4 * READ + WRITE)  ## numpy_math_expr_real_array_1d
            ]
...
#    complexity.cost(mode=mode, simplify=True, bigo=['n'])
    ...
    comp = [(ADD + MUL)/(3*READ + WRITE),# f1
            (ADD*n + DIV*n + MUL*n)/(4 * READ * (n - 1) + WRITE * (n - 1)),# f2
...
#    complexity.cost(mode=mode, simplify=True, bigo=['n'])
    ...
    comp = [(ADD + MUL)/(3*READ + WRITE),
            (n**3*(ADD + MUL))/((3*WRITE*b**2) + n**3*(3*READ + WRITE))
            ]
...
#    complexity.cost(mode=mode, simplify=True, bigo=['n'])
    ...
    print('----------------------')
    i = 0
    comp = [((ADD + MUL)/3)/(READ*n**3 + WRITE*n**3/3),
            ]
