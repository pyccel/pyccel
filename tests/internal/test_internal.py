...
@pytest.mark.external
def test_mpi(f):
    execute_pyccel(f, extra_compilation_tools = ['mpi'])
...
def test_openmp(f, language):
    execute_pyccel(f, extra_compilation_tools = ['openmp'], language = language)

#@pytest.mark.parametrize("f", get_files_from_folder('openacc'))
#@pytest.mark.external
#def test_openacc():
#    execute_pyccel(f, compiler='pgfortran', extra_compilation_tools = ['openacc'])
