...
def add_mixed_order(a : 'float[:,:]'):
    a[:] = a + b

def mul_mixed_order(a : 'float[:,:]'):
    a[:] = a * b

def sub_mixed_order(a : 'float[:,:]'):
    a[:] = a - b

def div_mixed_order(a : 'float[:,:]'):
    a[:] = a / b

def augadd_mixed_order(a : 'float[:,:]'):
    a[:] += b

def augmul_mixed_order(a : 'float[:,:]'):
    a[:] *= b

def augsub_mixed_order(a : 'float[:,:]'):
    a[:] -= b

def augdiv_mixed_order(a : 'float[:,:]'):
    a[:] /= b
...
def mul_by_vector_dim_1_F_C():
    import numpy as np
    b = np.array([[1],[2],[3]])
    a[:] *= b

def mul_by_vector_dim_1_F_F():
    import numpy as np
    b = np.array([[1],[2],[3]], order = 'F')
    a[:] *= b
