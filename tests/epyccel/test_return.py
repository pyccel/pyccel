# pylint: disable=missing-function-docstring, disable=unused-variable, missing-module-docstring/
from pyccel.decorators import types
from pyccel.epyccel import epyccel

def test_return(language):

    def single_return_var_assign():
        y = 3
        return y

    @types('int', 'int')
    def assign_vars_return(a,b):
        c = a+b
        d = a-b
        return c+d

    @types('int', 'int')
    def sum_in_single_return(a,b):
        c = a + b
        return c

    @types('int', 'int')
    def return_expr(x, y):
        return x + y

    @types('int')
    def return_single_var(x):
        return x

    def return_scalare():
        return 5

    def multi_return_scalare():
        return 5, 7

    @types('int', 'int')
    def multi_return_vars(a, b):
        return a, b

    @types('int', 'int')
    def multi_return_vars_expr(a, b):
        return (a-b), (a+b)

    @types('int')
    def scalare_multi_return_stmts(a):
        a = 7
        if a:
            return 1
        else:
            return 2
        a = 4
        return a

    def create_arr(i : int):
        import numpy as np
        a = np.ones(i)
        return True

    def return_arr_element(i : int):
        import numpy as np
        a = np.ones(i)
        return a[0]

    def create_multi_arrs(i : int):
        import numpy as np
        a = np.ones(i)
        b = np.zeros(i)
        c = np.zeros(i)
        return True

    def expr_arrs_elements(i : int):
        import numpy as np
        a = np.ones(i)
        b = np.zeros(i)
        return a[i - 1]+b[i - 1]

    def complex_expr(i : int):
        import numpy as np
        a = np.ones(i)
        return ((4 + 5)/(6 - 3) * a[0])%(9 - a[1])

    def multi_allocs(i :int):
        import numpy as np
        a = np.ones(i)
        b = np.ones(i)
        c = np.ones(i)
        d = np.ones(i)
        e = np.ones(i)
        return ((4 + 5)/(d[0] + e[2]) * c[0])%(b[2] + a[1]) - 4

    # these flags are used to make sure that Pyccel doesn't print an unused variable generated from a return statement
    flags = "-Werror -Wunused-variable"

    epyc_single_return_var_assign = epyccel(single_return_var_assign, language=language)
    assert (epyc_single_return_var_assign() == single_return_var_assign())

    epyc_assign_vars_return = epyccel(assign_vars_return, language=language, fflags=flags)
    assert (epyc_assign_vars_return(3, 4) == assign_vars_return(3, 4))

    epyc_sum_in_single_return = epyccel(sum_in_single_return, language=language, fflags=flags)
    assert (epyc_sum_in_single_return(7, 2) == sum_in_single_return(7, 2))

    epyc_return_expr = epyccel(return_expr, language=language, fflags=flags)
    assert (epyc_return_expr(7, 2) == return_expr(7, 2))

    epyc_return_single_var = epyccel(return_single_var, language=language, fflags=flags)
    assert (epyc_return_single_var(7) == return_single_var(7))

    epyc_return_scalare = epyccel(return_scalare, language=language, fflags=flags)
    assert (epyc_return_scalare() == return_scalare())

    epyc_multi_return_scalare = epyccel(multi_return_scalare, language=language, fflags=flags)
    assert (epyc_multi_return_scalare() == multi_return_scalare())

    epyc_multi_return_vars = epyccel(multi_return_vars, language=language, fflags=flags)
    assert (epyc_multi_return_vars(7, 2) == multi_return_vars(7, 2))

    epyc_multi_return_vars_expr = epyccel(multi_return_vars_expr, language=language, fflags=flags)
    assert (epyc_multi_return_vars_expr(7, 2) == multi_return_vars_expr(7, 2))

    epyc_scalare_multi_return_stmts = epyccel(scalare_multi_return_stmts, language=language, fflags=flags)
    assert (epyc_scalare_multi_return_stmts(7) == scalare_multi_return_stmts(7))

    epyc_create_arr = epyccel(create_arr, language=language, fflags=flags)
    assert (epyc_create_arr(7) == create_arr(7))

    epyc_return_arr_element = epyccel(return_arr_element, language=language, fflags=flags)
    assert (epyc_return_arr_element(7) == return_arr_element(7))

    epyc_create_multi_arrs = epyccel(create_multi_arrs, language=language, fflags=flags)
    assert (epyc_create_multi_arrs(7) == create_multi_arrs(7))

    epyc_expr_arrs_elements = epyccel(expr_arrs_elements, language=language, fflags=flags)
    assert (epyc_expr_arrs_elements(7) == expr_arrs_elements(7))

    epyc_complex_expr = epyccel(complex_expr, language=language, fflags=flags)
    assert (epyc_complex_expr(7) == complex_expr(7))

    epyc_multi_allocs = epyccel(multi_allocs, language=language, fflags=flags)
    assert (epyc_multi_allocs(7) == multi_allocs(7))
