...
def test_directive_in_else(language):
    f1 = epyccel(openmp.directive_in_else, flags = get_wall_flag(language), openmp=True, language=language)
    assert f1(0)  == 0
    assert f1(15) == 15
    assert f1(32) == 496
    assert f1(40) == 780
...
def test_module_2(language):
    f1 = epyccel(openmp.test_omp_number_of_procs, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_modules_4(lang):
    f1 = epyccel(openmp.test_omp_set_get_dynamic, flags = get_wall_flag(lang), openmp=True, language=lang)
...
def test_modules_4_1(lang):
    f1 = epyccel(openmp.test_omp_set_get_nested, flags = get_wall_flag(lang), openmp=True, language=lang)
...
def test_modules_5(language):
    f1 = epyccel(openmp.test_omp_get_cancellation, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_modules_6(language):
    f1 = epyccel(openmp.test_omp_get_thread_limit, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_modules_9(language):
    f1 = epyccel(openmp.test_omp_get_active_level, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_modules_7(language):
    f1 = epyccel(openmp.test_omp_get_set_max_active_levels, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_modules_8(language):
    f1 = epyccel(openmp.test_omp_get_level, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_modules_12(language):
    f1 = epyccel(openmp.test_omp_in_final, flags = get_wall_flag(language), openmp=True, language=language)

    assert f1() == 1

@pytest.mark.external
def test_modules_13(language):
    f1 = epyccel(openmp.test_omp_get_proc_bind, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_modules_15(language):
    f1 = epyccel(openmp.test_omp_get_team_num, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_modules_15_1(language):
    f1 = epyccel(openmp.test_omp_get_num_teams, flags = get_wall_flag(language), openmp=True, language=language)

    assert f1() == 2

@pytest.mark.external
def test_modules_16(language):
    f1 = epyccel(openmp.test_omp_get_max_task_priority, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_omp_range_sum_critical(language):
    f1 = epyccel(openmp.omp_range_sum_critical, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_omp_master(language):
    f1 = epyccel(openmp.omp_master, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_omp_taskloop(language):
    f1 = epyccel(openmp.omp_taskloop, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_omp_tasks(language):
    f1 = epyccel(openmp.omp_tasks, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_omp_flush(language):
    f1 = epyccel(openmp.omp_flush, flags = get_wall_flag(language), openmp=True, language=language)
    assert 2 == f1()

@pytest.mark.external
def test_omp_barrier(language):
    f1 = epyccel(openmp.omp_barrier, flags = get_wall_flag(language), openmp=True, language=language)
...
def test_omp_sections(language):
    f1 = epyccel(openmp.omp_sections, flags = get_wall_flag(language), openmp=True, language=language)
