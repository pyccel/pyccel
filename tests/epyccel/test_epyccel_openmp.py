...
def test_directive_in_else(language):
    f1 = epyccel(openmp.directive_in_else, flags = get_wall_flag(), openmp=True, language=language)
    assert f1(0)  == 0
    assert f1(15) == 15
    assert f1(32) == 496
    assert f1(40) == 780
...
def test_module_1(language):
    f1 = epyccel(openmp.f1, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads = epyccel(openmp.set_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    get_num_threads = epyccel(openmp.get_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    get_max_threads = epyccel(openmp.get_max_threads, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads(4)
...
def test_modules_10(language):
    set_num_threads = epyccel(openmp.set_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads(1)
    f1 = epyccel(openmp.test_omp_get_ancestor_thread_num, flags = get_wall_flag(), openmp=True, language=language)
...
def test_module_2(language):
    f1 = epyccel(openmp.test_omp_number_of_procs, flags = get_wall_flag(), openmp=True, language=language)
...
def test_module_3(language):
    set_num_threads = epyccel(openmp.set_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads(4)
    f1 = epyccel(openmp.test_omp_in_parallel1, flags = get_wall_flag(), openmp=True, language=language)
    f2 = epyccel(openmp.test_omp_in_parallel2, flags = get_wall_flag(), openmp=True, language=language)
...
def test_modules_4(lang):
    f1 = epyccel(openmp.test_omp_set_get_dynamic, flags = get_wall_flag(), openmp=True, language=lang)
...
def test_modules_4_1(lang):
    f1 = epyccel(openmp.test_omp_set_get_nested, flags = get_wall_flag(), openmp=True, language=lang)
...
def test_modules_5(language):
    f1 = epyccel(openmp.test_omp_get_cancellation, flags = get_wall_flag(), openmp=True, language=language)
...
def test_modules_6(language):
    f1 = epyccel(openmp.test_omp_get_thread_limit, flags = get_wall_flag(), openmp=True, language=language)
...
def test_modules_9(language):
    f1 = epyccel(openmp.test_omp_get_active_level, flags = get_wall_flag(), openmp=True, language=language)
...
def test_modules_7(language):
    f1 = epyccel(openmp.test_omp_get_set_max_active_levels, flags = get_wall_flag(), openmp=True, language=language)
...
def test_modules_8(language):
    f1 = epyccel(openmp.test_omp_get_level, flags = get_wall_flag(), openmp=True, language=language)
...
def test_modules_11(language):
    set_num_threads = epyccel(openmp.set_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads(4)
    f1 = epyccel(openmp.test_omp_get_team_size, flags = get_wall_flag(), openmp=True, language=language)
...
def test_modules_12(language):
    f1 = epyccel(openmp.test_omp_in_final, flags = get_wall_flag(), openmp=True, language=language)

    assert f1() == 1

@pytest.mark.external
def test_modules_13(language):
    f1 = epyccel(openmp.test_omp_get_proc_bind, flags = get_wall_flag(), openmp=True, language=language)
...
def test_modules_14_0(language):
    f1 = epyccel(openmp.test_omp_set_get_default_device, flags = get_wall_flag(), openmp=True, language=language)
    f2 = epyccel(openmp.test_omp_get_num_devices, flags = get_wall_flag(), openmp=True, language=language)
...
def test_modules_14_1(language):
    f3 = epyccel(openmp.test_omp_is_initial_device, flags = get_wall_flag(), openmp=True, language=language)
    f4 = epyccel(openmp.test_omp_get_initial_device, flags = get_wall_flag(), openmp=True, language=language) #Needs a non-host device to test the function properly
...
def test_modules_15(language):
    f1 = epyccel(openmp.test_omp_get_team_num, flags = get_wall_flag(), openmp=True, language=language)
...
def test_modules_15_1(language):
    f1 = epyccel(openmp.test_omp_get_num_teams, flags = get_wall_flag(), openmp=True, language=language)

    assert f1() == 2

@pytest.mark.external
def test_modules_16(language):
    f1 = epyccel(openmp.test_omp_get_max_task_priority, flags = get_wall_flag(), openmp=True, language=language)

    assert f1() == 0 # omp_get_max_task_priority() return always 0

@pytest.mark.external
def test_omp_matmul(language):
    f1 = epyccel(openmp.omp_matmul, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads = epyccel(openmp.set_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads(4)
    A1 = np.ones([3, 2])
    A1[1,0] = 2
...
def test_omp_matmul_single(language):
    f1 = epyccel(openmp.omp_matmul_single, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads = epyccel(openmp.set_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads(4)
    A1 = np.ones([3, 2])
    A1[1,0] = 2
...
def test_omp_nowait(language):
    f1 = epyccel(openmp.omp_nowait, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads = epyccel(openmp.set_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads(4)
...
def test_omp_arraysum(language):
    f1 = epyccel(openmp.omp_arraysum, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads = epyccel(openmp.set_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads(4)
...
def test_omp_arraysum_combined(language):
    f1 = epyccel(openmp.omp_arraysum_combined, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads = epyccel(openmp.set_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads(4)
    x = np.array(random.randint(20, size=(5)), dtype=int)
...
def test_omp_range_sum_critical(language):
    f1 = epyccel(openmp.omp_range_sum_critical, flags = get_wall_flag(), openmp=True, language=language)
...
def test_omp_arraysum_single(language):
    f1 = epyccel(openmp.omp_arraysum_single, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads = epyccel(openmp.set_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads(2)
...
def test_omp_master(language):
    f1 = epyccel(openmp.omp_master, flags = get_wall_flag(), openmp=True, language=language)
...
def test_omp_taskloop(language):
    f1 = epyccel(openmp.omp_taskloop, flags = get_wall_flag(), openmp=True, language=language)
...
def test_omp_tasks(language):
    f1 = epyccel(openmp.omp_tasks, flags = get_wall_flag(), openmp=True, language=language)
...
def test_omp_simd(language):
    f1 = epyccel(openmp.omp_simd, flags = get_wall_flag(), openmp=True, language=language)
    assert openmp.omp_simd(1337) == f1(1337)

@pytest.mark.external
def test_omp_long_line(language):
    f1 = epyccel(openmp.omp_long_line, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads = epyccel(openmp.set_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    set_num_threads(4)
...
def test_omp_flush(language):
    f1 = epyccel(openmp.omp_flush, flags = get_wall_flag(), openmp=True, language=language)
    assert 2 == f1()

@pytest.mark.external
def test_omp_barrier(language):
    f1 = epyccel(openmp.omp_barrier, flags = get_wall_flag(), openmp=True, language=language)
...
def test_combined_for_simd(language):
    ...
    f1 = epyccel(openmp.combined_for_simd, flags = get_wall_flag(), openmp=True, language=language, debug=False)
    f2 = openmp.combined_for_simd
    assert f1() == f2()

@pytest.mark.external
def test_omp_sections(language):
    f1 = epyccel(openmp.omp_sections, flags = get_wall_flag(), openmp=True, language=language)
...
def test_nowait_schedule(language):
    set_num_threads = epyccel(openmp.set_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    get_num_threads = epyccel(openmp.get_num_threads, flags = get_wall_flag(), openmp=True, language=language)
    f1 = epyccel(openmp.test_nowait_schedule, flags = get_wall_flag(), openmp=True, language=language)
