...
def skipif_by_language(request):
    ...
    if mark:
        ...
        assert "language" in mark.kwargs.keys(), (
            preamble + "you need to specify language=STRING"
        )
        assert "reason" in mark.kwargs.keys(), (
            preamble + "you need to specify reason=STRING"
        )
        ...
        assert isinstance(language, (str, tuple)), (
            preamble + "language must be str or tuple of str"
        )
    ...
    for root, _, files in os.walk(path_dir):
        for name in files:
            if name.startswith(".coverage"):
                shutil.copyfile(
                    os.path.join(root, name), os.path.join(os.getcwd(), name)
                )
...
def pytest_addoption(parser):
    parser.addoption(
        "--developer-mode",
        action="store_true",
        default=github_debugging,
        help="Show tracebacks when pyccel errors are raised",
    )
