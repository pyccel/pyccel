...
def get_functions(filename):
    ...
    pyccel = Parser(filename, output_folder=os.getcwd(), plugin_manager=plugin_manager)
    errors = Errors()
...
def test_get_all_user_nodes():
    ...
    assert all(
        isinstance(s, (PyccelMul, PyccelMinus, AugAssign, FunctionDefArgument))
        for s in users
    )
...
def test_get_direct_user_nodes():
    ...
    sums = set(
        a_var.get_direct_user_nodes(lambda x: isinstance(x, (PyccelAdd, PyccelMinus)))
    )
