...
int test_array_full_double(void)
{
    int m_1_shape[] = {5, 2};
    t_ndarray x;
    int index;
    ...
    x = array_create(2, m_1_shape, nd_double);
    for (int i = 0; i < x.length; i++)
        x.nd_double[i] = 1.;    // testing the index [3, 1]
    index = 3 * x.strides[0] + 1 * x.strides[1];
    c_index = 7;
    my_assert(index , c_index, "testing the strides");
    my_assert(get_index(x, 3, 1) , c_index, "testing the indexing function");
...
int test_array_full_cdouble(void)
{
    int m_1_shape[] = {5, 2};
    t_ndarray x;
    int index;
    ...
    x = array_create(2, m_1_shape, nd_cdouble);
    for (int i = 0; i < x.length; i++)
        x.nd_cdouble[i] = 1.+0*I;    // testing the index [3, 1]
    index = 3 * x.strides[0] + 1 * x.strides[1];
    c_index = 7;
    my_assert(index , c_index, "testing the strides");
    my_assert(get_index(x, 3, 1) , c_index, "testing the indexing function");
...
int test_array_zeros_double(void)
{
    ...
    x = array_create(2, m_1_shape, nd_double);
    memset(x.raw_data, 0, x.buffer_size);
    // testing the index [3, 1]
    index = 3 * x.strides[0] + 1 * x.strides[1];
    c_index = 7;
    my_assert(index , c_index, "testing the strides");
...
int main(void)
{
    ...
    /* array_full tests */
    test_array_full_int();
    test_array_full_double();
    test_array_full_cdouble();
    /* array_zeros tests */
