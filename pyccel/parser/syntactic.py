    ...
    **kwargs : dict
        ...
        if txt.startswith("$"):
            ...
            if env.startswith("omp"):
                ...
                try:
                    ...
                    errors.report(
                        f"Invalid OpenACC header. {e.message}",
                        symbol=stmt,
                        column=e.col,
                        severity="fatal",
                    )
    ...
    def _treat_type_annotation(self, stmt, annotation):
        ...
        if isinstance(annotation, (tuple, list)):
            return UnionTypeAnnotation(
                *[self._treat_type_annotation(stmt, a) for a in annotation]
            )
        if isinstance(annotation, (PyccelSymbol, DottedName, IndexedElement)):
            ...
            return UnionTypeAnnotation(
                *[self._treat_type_annotation(stmt, a) for a in annotation.args]
            )
        else:
            errors.report("Invalid type annotation", symbol=stmt, severity="error")
            return EmptyNode()

    def _get_unique_name(
        self, possible_names, valid_names, forbidden_names, suggestion
    ):
        """
        ...
        """
        if (
            all(isinstance(n, PythonTuple) for n in possible_names)
            and len(set(len(n) for n in possible_names)) == 1
        ):
            ...
            unique_names = [
                self._get_unique_name(
                    n, valid_names, forbidden_names, f"{suggestion}_{i}"
                )
            ...
            suggested_names = set(
                n for n in possible_names if isinstance(n, PyccelSymbol)
            )
    ...
    def _visit(self, stmt):
        ...
        if hasattr(self, syntax_method):
            ...
            try:
                ...
                if (
                    isinstance(result, PyccelAstNode)
                    and result.python_ast is None
                    and isinstance(stmt, ast.AST)
                ):
        ...
        return errors.report(
            PYCCEL_RESTRICTION_UNSUPPORTED_SYNTAX, symbol=stmt, severity="error"
        )
    ...
    def _visit_Module(self, stmt):
        ...
        body = [
            l
            for l in body
            if not isinstance(l, (FunctionDef, ClassDef, Import, Program))
        ]

        if len(programs) > 1:
            errors.report(
                "Multiple program blocks are not supported. Please group the code for the main.",
                symbol=programs[1],
                severity="error",
            )
    ...
    def _visit_AugAssign(self, stmt):
        ...
        if isinstance(stmt.op, ast.Add):
            ...
            return errors.report(PYCCEL_RESTRICTION_TODO, symbol=stmt, severity="error")
    ...
    def _visit_arguments(self, stmt):
        is_class_method = len(self._context) > 2 and isinstance(
            self._context[-3], ast.ClassDef
        )
        ...
        if stmt.posonlyargs:
            for a in stmt.posonlyargs:
                ...
                new_arg = FunctionDefArgument(
                    AnnotatedPyccelSymbol(a.arg, annotation),
        ...
        if stmt.args:
            ...
            for a in stmt.args[:n_expl]:
                annotation = self._treat_type_annotation(a, self._visit(a.annotation))
                new_arg = FunctionDefArgument(
                    AnnotatedPyccelSymbol(a.arg, annotation), annotation=annotation
                )
                new_arg.set_current_ast(a)
                arguments.append(new_arg)
        ...
        if stmt.vararg:
            annotation = self._treat_type_annotation(
                stmt.vararg, self._visit(stmt.vararg.annotation)
            )
            tuple_annotation = IndexedElement(
                PyccelSymbol("tuple"), annotation, LiteralEllipsis()
            )
            new_arg = FunctionDefArgument(
                AnnotatedPyccelSymbol(stmt.vararg.arg, tuple_annotation),
                annotation=annotation,
                is_vararg=True,
            )
        ...
        if is_class_method:
            ...
            if expected_self_arg.annotation is None:
                ...
                annotation = self._treat_type_annotation(
                    class_name, PyccelSymbol(class_name)
                )
                arguments[0] = FunctionDefArgument(
                    AnnotatedPyccelSymbol(expected_self_arg.name, annotation),
                    annotation=annotation,
        ...
        if stmt.kwarg:
            annotation = self._treat_type_annotation(
                stmt.kwarg, self._visit(stmt.kwarg.annotation)
            )
            dict_annotation = IndexedElement(
                PyccelSymbol("dict"), PyccelSymbol("str"), annotation
            )
            new_arg = FunctionDefArgument(
                AnnotatedPyccelSymbol(stmt.kwarg.arg, dict_annotation),
                annotation=annotation,
                is_kwarg=True,
            )
    ...
    def _visit_Constant(self, stmt):
        ...
        if stmt.value is None:
            ...
            raise NotImplementedError(
                f"Constant type {type(stmt.value)} not recognised"
            )
    ...
    def _visit_Import(self, stmt):
        ...
        for name in stmt.names:
            ...
            if isinstance(imp, AsName):
                source = AsName(
                    self._treat_import_source(imp.object, 0), imp.local_alias
                )
    ...
    def _visit_BoolOp(self, stmt):
        ...
        return errors.report(
            PYCCEL_RESTRICTION_UNSUPPORTED_SYNTAX, symbol=stmt.op, severity="error"
        )
    ...
    def _visit_FunctionDef(self, stmt):
        ...
        if classes:
            errors.report(
                "Classes in functions are not supported.",
                symbol=classes[0],
                severity="error",
            )
        ...
        targets = [
            t
            for target_list in self.scope.imports["imports"].values()
            for t in target_list
        ]

        returns = body.get_attribute_nodes(
            Return, excluded_nodes=(Assign, FunctionCall, PyccelFunction, FunctionDef)
        )
        if len(returns) == 0 or all(r.expr is Nil() for r in returns):
            ...
            results = self._get_unique_name(
                [r.expr for r in returns],
    ...
    def _visit_ClassDef(self, stmt):
        ...
        for i in stmt.body:
            ...
            if isinstance(visited_i, FunctionDef):
                ...
                continue
            elif isinstance(visited_i, CodeBlock) and all(
                isinstance(x, Comment) for x in visited_i.body
            ):
                continue
                ...
                errors.report(
                    f"{type(visited_i)} not currently supported in classes",
                    severity="error",
                    symbol=visited_i,
                )
        ...
        if init_method is None and not self.is_stub_file:
            ...
            self_arg = FunctionDefArgument(
                AnnotatedPyccelSymbol("self", annot),
                annotation=annot,
                kwonly=False,
                bound_argument=True,
            )
    ...
    def _visit_Call(self, stmt):
        ...
        if stmt.args:
            args += [
                FunctionCallArgument(self._visit(a), python_ast=a) for a in stmt.args
            ]
        if stmt.keywords:
        ...
        if len(args) == 1 and isinstance(
            args[0].value, (GeneratorComprehension, FunctionalFor)
        ):
            return args[0].value
        ...
        if isinstance(func, PyccelSymbol):
            if func == "print":
                ...
                if (
                    func in self._context_dict
                    and isinstance(self._context_dict[func], types.FunctionType)
    ...
    def _visit_comprehension(self, stmt):
        ...
        if stmt.ifs:
            cond = PyccelAnd(*[self._visit(if_cond) for if_cond in stmt.ifs])
    ...
    def _visit_ListComp(self, stmt):
        ...
        if not success and len(self._context) > 2:
            success = isinstance(self._context[-3], ast.Assign) and isinstance(
                self._context[-2], ast.Call
            )

        assignment = next(
            (c for c in reversed(self._context) if isinstance(c, ast.Assign)), None
        )
    ...
    def _visit_GeneratorExp(self, stmt):
        ...
        if name == "sum":
            expr = FunctionalSum(
                generators[0], result, lhs, indices, conditions=conditions
            )
        elif name == "min":
            expr = FunctionalMin(
                generators[0], result, lhs, indices, conditions=conditions
    ...
    def _visit_If(self, stmt):
        ...
        if (
            isinstance(test, PyccelEq)
            and test.args[0] == "__name__"
            and test.args[1] == "__main__"
            and isinstance(test.args[0], PyccelSymbol)
            and isinstance(test.args[1], LiteralString)
        ):
            if len(orelse) != 0:
                errors.report(
                    "Can't add an else condition to a program",
                    symbol=stmt,
                    severity="error",
                )
            ...
            if classes:
                errors.report(
                    "Classes should be declared in the module not in the program body",
                    symbol=stmt,
                    severity="error",
                )
            ...
            return Program(
                "__main__", (), CodeBlock(body), imports=imports, scope=scope
            )
        else:
            body = self._visit(stmt.body)
    ...
    def _visit_Lambda(self, stmt):
        ...
        if not isinstance(assign_node, ast.Assign):
            errors.report(
                "Lambda functions are only supported in assign statements currently.",
                severity="fatal",
                symbol=stmt,
            )
        ...
        return InlineFunctionDef(
            name, args, CodeBlock([return_expr]), results, scope=scope
        )
    ...
    def _visit_TypeAlias(self, stmt):
        if stmt.type_params:
            errors.report(
                "Type parameters are not yet supported on a type alias expression.\n"
                + PYCCEL_RESTRICTION_TODO,
                severity="error",
                symbol=stmt,
            )
        ...
        type_annotation = UnionTypeAnnotation(
