...
class SyntaxParser(BasicParser):
    ...
    **kwargs : dict
    ...
    def _textx_string_to_syntactic_AST(self, textx_str):
        """
        Convert a textx object to a syntactic AST object.

        Convert an object built by textx (a type annotation) into a syntactic AST
        object that can be passed to `_treat_type_annotation`.

        Parameters
        ----------
        textx_str : str | textx.Type
            The textx object being handled.

        Returns
        -------
        PyccelAstNode
            A syntactic AST object.
        """
        if textx_str == '...':
            return LiteralEllipsis()
        elif textx_str == ':':
            return Slice()
        elif isinstance(textx_str, str):
            return PyccelSymbol(textx_str)
        elif hasattr(textx_str, 'trailer'):
            trail = textx_str.trailer
            base = self._textx_string_to_syntactic_AST(textx_str.dtype)
            if trail:
                assert not textx_str.trailer.order
                idxs = [self._textx_string_to_syntactic_AST(t) for t in textx_str.trailer.args]
                return IndexedElement(base, *idxs)
            else:
                return base
        else:
            raise NotImplementedError(textx_str)

    def _build_annotation_from_textx(self, annotation, ast_node):
        """
        ...
        if isinstance(annotation, (list, tuple)):
            return tuple(self._build_annotation_from_textx(a, ast_node) for a in annotation)
        elif hasattr(annotation, 'const'):
            # Handle UnionTypeStmt
            is_const = annotation.const
            dtypes = [self._build_annotation_from_textx(a, ast_node) for a in annotation.dtypes]
            ...
            # Handle VariableType
            is_const = None
            trailing_args = getattr(annotation.trailer, 'args', ())
            if all(a == ':' for a in trailing_args):
                dtype_names = [annotation.dtype]
                ranks = [len(trailing_args)]
                orders = [getattr(annotation.trailer, 'order', None)]
                return SyntacticTypeAnnotation(dtype_names, ranks, orders, is_const)
            else:
                idxs = [self._textx_string_to_syntactic_AST(a) for a in annotation.trailer.args]
                annot = IndexedElement(self._textx_string_to_syntactic_AST(annotation.dtype), *idxs)
                if isinstance(ast_node, PyccelAstNode):
                    annot.set_current_ast(ast_node.python_ast)
                else:
                    annot.set_current_ast(ast_node)
                return self._treat_type_annotation(ast_node, annot)
        elif hasattr(annotation, 'results'):
            # Handle FuncType
            args = [self._build_annotation_from_textx(a, ast_node) for a in annotation.decs]
            results = [self._build_annotation_from_textx(r, ast_node) for r in annotation.results]
