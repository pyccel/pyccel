...
def insert_comments(
    ast, comment_lines_no, comments, else_no, attr="body", col_offset=None
):
    if len(comments) == 0:
        return
    ...
    while ind < len(body) - 1 and len(comments) > 0:
        ...
        if not hasattr(previous_stmt, "body"):
            ...
            if orelse:
                ...
                elif_orelse = len(previous_stmt.orelse) == 1 and isinstance(
                    previous_stmt.orelse[0], IfNode
                )
            else:
            ...
            for k, comment_line_no_k in enumerate(comment_lines_no):
                if previous_stmt_body_last_lineno < comment_line_no_k:
                    if orelse and elif_orelse:
                        ...
                        col_offset >= comments[k].col_offset
                        and not orelse
                        or next_node_lineno < comment_line_no_k
                    ):
                        break
            else:
                k = k + 1
            ...
            if orelse and not elif_orelse and k > 0:
                expr = logical_and(
                    else_no <= comment_lines_no[k - 1],
                    else_no <= previous_stmt_body_last_lineno,
                )
                if expr.any():
            ...
            if orelse:
                previous_stmt_orelse_last_lineno = get_last_lineno(
                    previous_stmt.orelse[-1]
                )
                # TODO accelerate this part with pyccel
                k = -1
                for k, comment_line_no_k in enumerate(comment_lines_no):
    ...
    if hasattr(last_stmt, "body"):
        ...
        if orelse:
            ...
            elif_orelse = len(last_stmt.orelse) == 1 and isinstance(
                last_stmt.orelse[0], IfNode
            )
        else:
        ...
        if orelse and not elif_orelse and k > 0:
            expr = logical_and(
                else_no <= comment_lines_no[k - 1], else_no <= body_last_lineno
            )
            if expr.any():
                k = where(else_no[expr][-1] <= comment_lines_no[:k])[0][0]
