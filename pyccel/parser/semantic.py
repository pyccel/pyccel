...
class SemanticParser(BasicParser):
    ...
    **kwargs : dict
    ...
    def __init__(
        self, inputs, *, parents=(), d_parsers=(), context_dict=None, **kwargs
    ):
    ...
    def get_class_prefix(self, name):
        ...
        syntactic_prefix = (
            prefix_parts[0] if len(prefix_parts) == 1 else DottedName(*prefix_parts)
        )
        ...
        if isinstance(name, DottedName):
            ...
            if attribute:
                return attribute.clone(
                    attribute.name, new_class=DottedVariable, lhs=prefix
                )
            else:
                return None
    ...
    def insert_import(self, name, target, storage_name=None):
        ...
        if storage_name is None:
            storage_name = next(
                (
                    n
                    for n, imp in self.scope.find_all("imports").items()
        ...
        if imp is not None:
            if found_from_import_name or source in (
                imp.source,
                getattr(imp.source_module, "name", ""),
            ):
    ...
    def create_tuple_of_inhomogeneous_elements(self, tuple_var):
        ...
        if isinstance(tuple_var.class_type, InhomogeneousTupleType):
            return PythonTuple(
                *[
                    self.create_tuple_of_inhomogeneous_elements(
                        self.scope.collect_tuple_element(v)
                    )
                    for v in tuple_var
                ]
            )
        ...
        if all(r.expr is None for r in expr.get_attribute_nodes(Return)):
            for i in self._allocs[-1]:
                if isinstance(i, DottedVariable):
                    if (
                        isinstance(i.lhs.class_type, CustomDataType)
                        and self.current_function_name != "__del__"
                    ):
                        continue
    ...
    def _check_pointer_targets(self, exceptions=()):
        ...
        targets = {
            t[0]: t[1]
            for target_list in self._pointer_targets[-1].values()
        ...
        for i in self._allocs[-1]:
            ...
            if i in targets:
                errors.report(
                    f"Variable {i} goes out of scope but may be the target of a pointer which is still required",
                    severity="error",
                    symbol=targets[i],
                )
        ...
        if self.current_function_name:
            ...
            for p, t_list in self._pointer_targets[-1].items():
                if p in arg_vars and arg_vars[p].bound_argument:
                    for t, _ in t_list:
                        if t.is_argument:
                            argument_objects = t.get_direct_user_nodes(
                                lambda x: isinstance(x, FunctionDefArgument)
                            )
    ...
    def _indicate_pointer_target(self, pointer, target, expr):
        ...
        pointing_at_container_element = (
            isinstance(pointer.class_type, (HomogeneousSetType, HomogeneousListType))
            and (target.class_type is pointer.class_type.element_type)
        ) or (
            isinstance(pointer.class_type, DictType)
            and (target.class_type is pointer.class_type.value_type)
        )
        container_pointing_at_element = (
            isinstance(target.class_type, (HomogeneousSetType, HomogeneousListType))
            and (pointer.class_type is target.class_type.element_type)
        ) or (
        ...
        if pointing_at_container_element or container_pointing_at_element:
            ...
            if isinstance(managed_var, Variable):
                managed_mem = managed_var.get_direct_user_nodes(
                    lambda u: isinstance(u, ManagedMemory)
                )
                if not managed_mem:
                    mem_var = Variable(
                        MemoryHandlerType.get_new(managed_var.class_type),
        ...
        if isinstance(target, DottedVariable):
            ...
            if target.is_alias:
                ...
                try:
                    ...
                    errors.report(
                        "Pointer cannot point at a non-local pointer\n"
                        + PYCCEL_RESTRICTION_TODO,
                        severity="error",
                        symbol=expr,
                    )
                if sub_targets:
                    self._pointer_targets[-1].setdefault(pointer, []).extend(
                        (t[0], expr) for t in sub_targets
                    )
            else:
            ...
            if not isinstance(
                target.class_type.element_type, (StringType, FixedSizeNumericType)
            ):
                for v in target:
                    self._indicate_pointer_target(pointer, v, expr)
        elif isinstance(target, (ListPop, DictPop)):
            target_var = (
                target.list_obj if isinstance(target, ListPop) else target.dict_obj
            )
            ...
            if isinstance(pointer, Variable):
                managed_mem = pointer.get_direct_user_nodes(
                    lambda u: isinstance(u, ManagedMemory)
                )
                if not managed_mem:
                    mem_var = Variable(
                        MemoryHandlerType.get_new(pointer.class_type),
            ...
            if not isinstance(
                target.class_type.value_type, (StringType, FixedSizeNumericType)
            ):
    ...
    def _infer_type(self, expr):
        ...
        if isinstance(class_type, FinalType) and isinstance(
        ...
        if isinstance(expr, Variable):
            ...
            d_var["memory_handling"] = (
                "alias" if not isinstance(class_type, FixedSizeNumericType) else "stack"
            )
            return d_var
    ...
    def _extract_indexed_from_var(self, var, indices, expr):
        ...
        if isinstance(var, PythonTuple):
            ...
            def is_literal_index(a):
                ...
                if isinstance(a, Slice):
                    return all(
                        is_int(s) or s is None for s in (a.start, a.step, a.stop)
                    )
                else:
                    return is_int(a)
            ...
            errors.report(f"Can't index {type(var)}", symbol=expr, severity="fatal")
        ...
        if isinstance(var.class_type, InhomogeneousTupleType):
            ...
            if isinstance(arg, Slice):
                ...
                if len(indices) == 1:
                    ...
                    return PythonTuple(
                        *[
                            self._extract_indexed_from_var(var, indices[1:], expr)
        ...
        for arg in var[indices].indices:
            if not isinstance(arg, (Slice, LiteralEllipsis)) and not (
                hasattr(arg, "dtype")
                and isinstance(
                    getattr(arg.dtype, "primitive_type", None), PrimitiveIntegerType
                )
            ):
                errors.report(
                    INVALID_INDICES,
    ...
    def _create_PyccelOperator(self, expr, visited_args):
        ...
        if all(isinstance(a, PyccelFunctionDef) for a in visited_args):
            try:
                ...
                errors.report(
                    "Unrecognised type in type union statement",
                    severity="fatal",
                    symbol=expr,
                )
            return UnionTypeAnnotation(
                *[VariableTypeAnnotation(t) for t in possible_types]
            )
        ...
        if magic_method:
            expr_new = self._handle_function(
                expr, magic_method, [FunctionCallArgument(v) for v in visited_args]
            )
        else:
            try:
                ...
                errors.report(
                    f"Operator {type(expr)} between objects of type ({types}) is not yet handled\n"
                    + PYCCEL_RESTRICTION_TODO,
                    symbol=expr,
                    severity="fatal",
        ...
        if isinstance(val.class_type, (HomogeneousTupleType, HomogeneousListType)):
            ...
            if isinstance(length, LiteralInteger):
                ...
                if isinstance(sympy_length, sp_Integer):
                    ...
                    errors.report(
                        "Cannot create inhomogeneous tuple of unknown size",
                        symbol=Duplicate(val, length),
                        bounding_box=(
                            self.current_ast_node.lineno,
                            self.current_ast_node.col_offset,
            ...
            return PythonTuple(
                *([self.scope.collect_tuple_element(v) for v in val] * length)
            )
    ...
    def _create_class_destructor(self, expr):
        ...
        if del_method is None:
            ...
            argument = FunctionDefArgument(
                Variable(class_type, scope.get_new_name("self"), cls_base=expr),
                bound_argument=True,
            )
    ...
    def _handle_function_args(self, arguments):
        ...
        for arg in arguments:
            ...
            val = a.value
            if (
                isinstance(val, FunctionDef)
                and not isinstance(val, PyccelFunctionDef)
                and not val.is_semantic
            ):
                semantic_func = self._annotate_the_called_function_def(val, ())
                a = FunctionCallArgument(
                    semantic_func, keyword=a.keyword, python_ast=a.python_ast
                )
    ...
    def _check_argument_compatibility(
        ...
        for idx, (i_arg, f_arg) in enumerate(zip(input_args, func_args)):
            ...
            if incompatible(i_arg, f_arg):
                ...
                err_msgs += [
                    INCOMPATIBLE_ARGUMENT.format(idx + 1, received, func, expected)
                ]
    ...
    def _handle_function(
        self, expr, func, args, *, is_method=False, use_build_functions=True
    ):
        """
        ...
        if isinstance(func, PyccelFunctionDef):
            if use_build_functions:
                ...
                annotation_method = next(
                    (
                        f"_build_{cls.__name__}"
                        for cls in classes
                        if hasattr(self, f"_build_{cls.__name__}")
                    ),
                    None,
                )
                if annotation_method:
                    if isinstance(expr, DottedName):
                        ...
                        if is_method:
                            new_expr = DottedName(
                                args[0].value, FunctionCall(func, args[1:])
                            )
                        else:
                            new_expr = FunctionCall(func, args)
                        new_expr.set_current_ast(
                            expr.python_ast or self._current_ast_node
                        )
                        pyccel_stage.set_stage("semantic")
            ...
            try:
                ...
                errors.report(
                    message, symbol=expr, traceback=e.__traceback__, severity="fatal"
                )
            ...
            if self.current_function_name == func.name:
                if func.results and not isinstance(func.results.var, TypedAstNode):
                    errors.report(
                        RECURSIVE_RESULTS_REQUIRED, symbol=func, severity="fatal"
                    )

            parent_assign = expr.get_direct_user_nodes(
                lambda x: isinstance(x, Assign) and not isinstance(x, AugAssign)
            )

            func_results = (
                func.results
                if isinstance(func, FunctionDef)
                else func.functions[0].results
            )
            ...
            func_args = (
                func.arguments
                if isinstance(func, FunctionDef)
                else func.functions[0].arguments
            )
            ...
            for a, f_a in zip(new_expr.args, func_args):
                if f_a.persistent_target:
                    ...
                    if isinstance(val, Variable):
                        ...
                        errors.report(
                            f"{val} cannot be passed to function call as target. Please create a temporary variable.",
                            severity="error",
                            symbol=expr,
                        )
    ...
    def _sort_function_call_args(self, func_args, args):
        ...
        n_posonly_args = next(
            (i for i, a in enumerate(func_args) if not a.is_posonly), n_funcargs
        )
        n_possible_posargs = next(
            (i for i, a in enumerate(func_args) if a.is_vararg), n_funcargs
        )
        input_args = []
        kwargs = {}
        for i, a in enumerate(args):
            if a.keyword is None:
                if (
                    isinstance(a.value, StarredArguments)
                    and not func_args[len(input_args)].is_vararg
                ):
                    input_args.extend(
                        FunctionCallArgument(self.scope.collect_tuple_element(av))
        ...
        if len(input_args) > n_possible_posargs:
            if len(input_args) == n_possible_posargs + 1 and isinstance(
                input_args[-1].value, StarredArguments
            ):
                vararg = FunctionCallArgument(
                    input_args[-1].value.args_var, keyword="*args"
                )
            else:
                remaining_input_args = [
                    ai.value
                    for a in input_args[n_possible_posargs:]
                    for ai in (a.args_var if isinstance(a, StarredArguments) else (a,))
                ]
                vararg = FunctionCallArgument(
                    PythonTuple(*remaining_input_args), keyword="*args"
                )
        ...
        for ka in func_args[len(input_args) :]:
            if ka.is_vararg:
                input_args.append(
                    FunctionCallArgument(
                        PythonTuple(class_type=ka.var.class_type), keyword="*args"
                    )
                )
                continue
            if not ka.is_kwarg:
        ...
        if kwargs:
            ...
            input_args.append(
                FunctionCallArgument(PythonDict(keys, values), keyword="**kwargs")
            )
    ...
    def _annotate_the_called_function_def(self, old_func, function_call_args):
        ...
        if cls_base_syntactic:
            ...
            func_scope = (
                old_func.scope
                if isinstance(old_func, FunctionDef)
                else old_func.syntactic_node.scope
            )
    ...
    def is_memory_allocated_in_function(self, lhs, rhs, heap_mem_in_multirets):
        ...
        return not isinstance(lhs.class_type, FixedSizeNumericType) and (
            (
                ...
                and not isinstance(rhs.funcdef, PyccelFunctionDef)
                and not getattr(rhs.funcdef, "is_elemental", False)
                and not isinstance(lhs.class_type, HomogeneousTupleType)
            )
            or heap_mem_in_multirets
            or isinstance(
                rhs, (ListPop, SetPop, DictPop, DictPopitem, DictGet, DictGetItem)
            )
        )
        ...
        *,
        ...
        if isinstance(class_type, InhomogeneousTupleType):
            ...
            if isinstance(rhs, FunctionCall):
                ...
                iterable = [
                    rhs_scope.collect_tuple_element(v) for v in rhs.funcdef.results.var
                ]
    ...
    def _ensure_target(self, rhs, d_lhs):
        ...
        if not isinstance(
            rhs.class_type, (TupleType, StringType, FixedSizeNumericType)
        ):
            if isinstance(rhs, Variable):
                d_lhs["memory_handling"] = "alias"
                rhs.is_target = not rhs.is_alias

            elif isinstance(rhs, IndexedElement) and isinstance(
                rhs.class_type, (HomogeneousTupleType, NumpyNDArrayType)
            ):
                d_lhs["memory_handling"] = "alias"
    ...
    def _assign_lhs_variable(
        ...
        if isinstance(lhs, IndexedElement):
            ...
            if isinstance(lhs, DottedName):
                ...
                attribute = (
                    class_def.scope.variables.get(attr_name, None)
                    if class_def
                    else None
                )
                if attribute:
                ...
                if symbolic_var:
                    errors.report(
                        f"Variable '{lhs}' represents a symbolic concept. Its value cannot be changed.",
                        severity="fatal",
                        bounding_box=(
                            self.current_ast_node.lineno,
                            self.current_ast_node.col_offset,
            ...
            if var is None:
                ...
                if isinstance(lhs, DottedName):
                    ...
                    if prefix.name == "self":
                        ...
                        errors.report(
                            f"{lhs.name[0]} should be named : self",
                            symbol=lhs,
                            severity="fatal",
                        )
                ...
                if (
                    class_type.rank > 0
                    and d_lhs.get("memory_handling", None) == "stack"
                ):
                    for a in d_lhs["shape"]:
                        if (
                            isinstance(a, FunctionCall) and not a.funcdef.is_pure
                        ) or any(
                            not f.funcdef.is_pure
                            for f in a.get_attribute_nodes(FunctionCall)
                        ):
                            errors.report(
                                STACK_ARRAY_SHAPE_UNPURE_FUNC,
                                symbol=a.funcdef.name,
                                severity="error",
                                bounding_box=(
                                    self.current_ast_node.lineno,
                                    self.current_ast_node.col_offset,
                        ...
                        if (isinstance(a, Variable) and not a.is_argument) or not all(
                            b.is_argument for b in a.get_attribute_nodes(Variable)
                        ):
                            errors.report(
                                STACK_ARRAY_UNKNOWN_SHAPE,
                                symbol=name,
                ...
                if not isinstance(lhs, DottedVariable):
                    ...
                    # Create new variable
                    lhs = self._create_variable(
                        new_name,
                        class_type,
                        rhs,
                        d_lhs,
                        heap_mem_in_multirets=heap_mem_in_multirets,
                    )
                ...
                memory_allocated_in_function = self.is_memory_allocated_in_function(
                    lhs, rhs, heap_mem_in_multirets
                )
                if lhs.on_heap:
                    ...
                    if isinstance(lhs.class_type, InhomogeneousTupleType):
                        ...
                        if alloc_type is None:
                            if isinstance(
                                rhs, (PythonList, PythonDict, PythonSet, FunctionCall)
                            ):
                                alloc_type = "init"
                ...
                if memory_allocated_in_function:
                    ...
                    know_lhs_shape = (lhs.rank == 1) or all(
                        sh is not None for sh in lhs.alloc_shape[:-1]
                    )
                else:
                    know_lhs_shape = (lhs.rank == 0) or all(
                        sh is not None for sh in lhs.alloc_shape
                    )

                if (
                    isinstance(class_type, (NumpyNDArrayType, HomogeneousTupleType))
                    and not know_lhs_shape
            ...
            # Variable already exists
                ...
                if isinstance(var, Constant):
                    errors.report(
                        f"Attempting to overwrite the constant {lhs}",
                        bounding_box=(
                            self.current_ast_node.lineno,
                            self.current_ast_node.col_offset,
                        ),
                ...
                if isinstance(var, DottedVariable):
                    var = next(
                        (a for a in var.lhs.cls_base.attributes if var == a), var
                    )

                self._ensure_inferred_type_matches_existing(
                    class_type,
                    d_lhs,
                    var,
                    is_augassign,
            ...
            raise NotImplementedError(
                f"_assign_lhs_variable does not handle {lhs_type}"
            )

        return lhs

    def _ensure_inferred_type_matches_existing(
        self,
        class_type,
        d_var,
        var,
        ...
        is_reallocatable_container = not isinstance(
            var.class_type, FixedSizeNumericType
        )
        if not is_augassign and is_reallocatable_container and var.is_target:
            pointers = [
                p
                for pt in self._pointer_targets
                for p, t in pt.items()
            ...
            and var.rank > 0
            and isinstance(rhs, (Variable, IndexedElement))
            and not isinstance(var.class_type, (StringType, TupleType))
            and not heap_mem_in_multirets
        ):
            errors.report(
                ASSIGN_ARRAYS_ONE_ANOTHER,
                bounding_box=(
            ...
            var.rank > 0
            and var.is_alias
            and isinstance(rhs, (NumpyNewArray, PythonList, PythonSet, PythonDict))
        ):
            errors.report(
                INVALID_POINTER_REASSIGN,
                bounding_box=(
            ...
            and class_type is not var.class_type
        ):
            if is_augassign:
                tmp_result = PyccelAdd(var, rhs)
                result_type = tmp_result.class_type
                raise_error = var.class_type != result_type
            elif isinstance(var.class_type, InhomogeneousTupleType) and isinstance(
                ...
                class_type, InhomogeneousTupleType
            ):
                ...
                raise_error = False
            elif isinstance(var.class_type, HomogeneousTupleType) and isinstance(
                class_type, InhomogeneousTupleType
            ):
                ...
                if isinstance(rhs, PythonTuple):
                    ...
                    raise_error = (
                        len(shape) != class_type.rank
                        or not class_type.shape_is_compatible(shape)
                        or any(a != var.class_type.element_type for a in class_type)
                    )
                else:
                    raise_error = any(
                        a != var.class_type.element_type for a in class_type
                    ) or not isinstance(rhs, Variable)
            else:
                raise_error = True
            ...
            if raise_error:
                ...
                rhs_str = str(rhs)
                errors.report(
                    INCOMPATIBLE_TYPES_IN_ASSIGNMENT.format(var.class_type, class_type),
        ...
        if not is_augassign:
            ...
            previous_allocations = var.get_direct_user_nodes(
                lambda p: isinstance(p, Allocate)
            )
            ...
            memory_allocated_in_function = self.is_memory_allocated_in_function(
                var, rhs, heap_mem_in_multirets
            )

            if (
                d_var["shape"] != shape
                or memory_allocated_in_function
                or (
                    shape
                    and not all(isinstance(s, LiteralInteger) for s in shape)
                ...
                if var.is_argument:
                    ...
                    if var.get_direct_user_nodes(
                        lambda a: isinstance(a, Assign) and a.lhs is var
                    ):
                        errors.report(
                            INCOMPATIBLE_REDEFINITION_STACK_ARRAY,
                            symbol=var.name,
                    ...
                    if previous_allocations:
                        ...
                        if isinstance(var, DottedVariable):
                            ...
                            attribute = var.lhs.cls_base.scope.find(
                                py_name, "variables"
                            )
                        ...
                        alloc_ifsection = (
                            last_alloc_ifsection[-1] if last_alloc_ifsection else None
                        )
                        while len(last_alloc_ifsection) > 0:
                            alloc_ifsection = last_alloc_ifsection[-1]
                            last_alloc_ifsection = alloc_ifsection.get_user_nodes(
                                IfSection
                            )

                        ifsection_has_if = (
                            len(
                                alloc_ifsection.get_direct_user_nodes(
                                    lambda x: isinstance(x, If)
                                )
                            )
                            == 1
                            if alloc_ifsection
                    ...
                    if status == "unallocated":
                        ...
                        errors.report(
                            ARRAY_REALLOCATION.format(class_type=var.class_type),
                            symbol=var.name,
                            severity="warning",
                            bounding_box=(
                                self.current_ast_node.lineno,
                ...
                and previous_allocations[-1].get_user_nodes(IfSection)
                ...
                new_expressions.append(
                    Allocate(var, shape=d_var["shape"], status=status)
                )
    ...
    def _assign_GeneratorComprehension(self, lhs_name, expr):
        ...
        index = Variable(
            PythonNativeInt(), self.scope.get_new_name("to_delete"), is_temp=True
        )
        ...
        if isinstance(expr, FunctionalSum):
            expr_new = FunctionalSum(
                loops, lhs=lhs, indices=indices, conditions=expr.conditions
            )
        elif isinstance(expr, FunctionalMin):
            expr_new = FunctionalMin(
                loops, lhs=lhs, indices=indices, conditions=expr.conditions
            )
        elif isinstance(expr, FunctionalMax):
            expr_new = FunctionalMax(
                loops, lhs=lhs, indices=indices, conditions=expr.conditions
            )
    ...
    def _find_superclasses(self, expr):
        ...
        if any(c is None for c in parent.values()):
            for s, c in parent.items():
                if c is None:
                    errors.report(
                        f"Couldn't find class {s} in scope",
                        symbol=expr,
                        severity="error",
                    )
    ...
    def _get_indexed_type(self, base, args, expr):
        ...
        if isinstance(base, UnionTypeAnnotation):
            return UnionTypeAnnotation(
                *[self._get_indexed_type(t, args, expr) for t in base.type_list]
            )

        if isinstance(base, PyccelFunctionDef) and base.cls_name is TypingAnnotation:
            annotation = self._visit(
                self._convert_syntactic_object_to_type_annotation(args[0])
            )
            if any(
                isinstance(a.class_type, TypingAnnotation) for a in annotation.type_list
            ):
                errors.report(
            ...
            metavars = [
                (
                    self._visit(a.dtype)
                    if isinstance(a, SyntacticTypeAnnotation)
                    else self._visit(a)
                )
                for a in args[1:]
            ]
            ...
            if "stack" in metavars:
                if "memory_handling" in var_metadata:
                    errors.report(
                        "An object cannot be both an alias for an object stored elsewhere and a stack allocated object.",
                        bounding_box=(
                            self.current_ast_node.lineno,
                            self.current_ast_node.col_offset,
                        ),
            ...
            if metavars:
                errors.report(
                    f"Unrecognised annotations have been ignored ({metavars})",
                    bounding_box=(
                        self.current_ast_node.lineno,
                        self.current_ast_node.col_offset,
                    ),
            ...
            if var_metadata:
                ...
                assert all(
                    isinstance(t, VariableTypeAnnotation) for t in annotation.type_list
                )
                return UnionTypeAnnotation(
                    *[TypingAnnotation(d, **var_metadata) for d in annotation.type_list]
                )
            else:
                return annotation
        ...
        if len(args) == 0:
            if not (
                isinstance(base, PyccelFunctionDef)
                and base.cls_name.static_type() is TupleType
            ):
                errors.report("Unrecognised type", severity="fatal", symbol=expr)
            return UnionTypeAnnotation(
                VariableTypeAnnotation(InhomogeneousTupleType.get_new())
            )
        ...
        if all(isinstance(a, Slice) for a in args):
            ...
            if isinstance(base, VariableTypeAnnotation):
                ...
                if dtype.rank != 0:
                    raise errors.report(
                        "NumPy element must be a scalar type",
                        severity="fatal",
                        symbol=expr,
                    )
                class_type = NumpyNDArrayType.get_new(
                    numpy_process_dtype(dtype), rank, order
                )
                ...
                try:
                    ...
                    errors.report(
                        f"Unrecognised datatype {dtype_cls}",
                        severity="fatal",
                        symbol=expr,
                    )
        ...
        if not any(isinstance(a, Slice) for a in args):
            if isinstance(base, PyccelFunctionDef):
                ...
                raise errors.report(
                    f"Unknown annotation base {base}\n" + PYCCEL_RESTRICTION_TODO,
                    severity="fatal",
                    symbol=expr,
                )
            if (len(args) == 2 and args[1] is LiteralEllipsis()) or (
                ...
                class_type = (
                    HomogeneousTupleType if dtype_cls is TupleType else dtype_cls
                )
                type_annotations = [
                    VariableTypeAnnotation(class_type.get_new(u.class_type))
                ...
                syntactic_key_annotation = (
                    self._convert_syntactic_object_to_type_annotation(args[0])
                )
                syntactic_val_annotation = (
                    self._convert_syntactic_object_to_type_annotation(args[1])
                )
                key_types = self._visit(syntactic_key_annotation)
                val_types = self._visit(syntactic_val_annotation)
                type_annotations = [
                    VariableTypeAnnotation(
                        dtype_cls.get_new(k.class_type, v.class_type)
                    )
                ...
                syntactic_annotations = [
                    self._convert_syntactic_object_to_type_annotation(a) for a in args
                ]
                types = [self._visit(a).type_list for a in syntactic_annotations]
                internal_datatypes = list(product(*types))
                type_annotations = [
                    VariableTypeAnnotation(
                        InhomogeneousTupleType.get_new(*[ui.class_type for ui in u])
                    )
                ...
                raise errors.report(
                    "Cannot handle non-homogenous type index\n"
                    + PYCCEL_RESTRICTION_TODO,
                    severity="fatal",
                    symbol=expr,
                )

        raise errors.report("Unrecognised type slice", severity="fatal", symbol=expr)
    ...
    def insert_attribute_to_class(self, class_def, self_var, attrib):
        ...
        if isinstance(attrib.class_type, InhomogeneousTupleType):
            for v in attrib:
                self.insert_attribute_to_class(
                    class_def, self_var, class_def.scope.collect_tuple_element(v)
                )
        else:
    ...
    def _get_iterable(self, syntactic_iterable):
        ...
        if isinstance(iterable, (Variable, IndexedElement)):
            ...
            if isinstance(iterable, TypedAstNode):
                ...
                syntactic_assign = Assign(
                    tmp_var, iterable, python_ast=iterable.python_ast
                )
                pyccel_stage.set_stage("semantic")
                assign = self._visit(syntactic_assign)
                ...
                errors.report(
                    f"{iterable} is not handled as the iterable of a for loop",
                    symbol=syntactic_iterable,
                    severity="fatal",
                )
    ...
    def _get_for_iterators(self, syntactic_iterable, iterator, new_expr, expr):
        ...
        if iterable.num_loop_counters_required:
            indices = [
                Variable(PythonNativeInt(), self.scope.get_new_name(), is_temp=True)
            ...
            if index is None:
                ...
                if isinstance(syntactic_index, PyccelSymbol):
                    index = self._assign_lhs_variable(
                        syntactic_index, d_var, rhs=start, new_expressions=new_expr
                    )
                else:
        ...
        if isinstance(iterator, PyccelSymbol):
            ...
            target = self._assign_lhs_variable(
                iterator, iterator_d_var, rhs=iterator_rhs, new_expressions=new_expr
            )
            ...
            target = [
                self._assign_lhs_variable(
                    it, self._infer_type(rhs), rhs=rhs, new_expressions=new_expr
                )
    ...
    def env_var_to_pyccel(self, env_var, *, name=None):
        ...
        if env_var in original_type_to_pyccel_type:
            ...
            return convert_to_literal(
                env_var, dtype=original_type_to_pyccel_type[type(env_var)]
            )
            ...
            class_type = self.env_var_to_pyccel(
                typing.get_origin(env_var)
            ).class_type.static_type()
            return VariableTypeAnnotation(
                class_type.get_new(
                    *[
                        self.env_var_to_pyccel(a).class_type
                        for a in typing.get_args(env_var)
                    ]
                )
            )
        elif isinstance(env_var, typing.TypeVar):
            constraints = [self.env_var_to_pyccel(c) for c in env_var.__constraints__]
            return TypingTypeVar(
                env_var.__name__,
                *constraints,
            ...
            if recognised_source(mod_name):
                ...
                errors.report(
                    f"Unrecognised module {mod_name} imported in global scope. Please import the module locally if it was previously Pyccelised.",
                    severity="error",
                    symbol=self.current_ast_node,
                )
        elif isinstance(env_var, (typing.ForwardRef, str)):
            pyccel_stage.set_stage("syntactic")
            try:
                annotation = types_meta.model_from_str(
                    getattr(env_var, "__forward_arg__", env_var)
                )
            except TextXSyntaxError as e:
                errors.report(
                    f"Invalid annotation. {e.message}",
                    symbol=self.current_ast_node,
                    severity="fatal",
                )
        ...
        errors.report(
            PYCCEL_RESTRICTION_TODO, severity="error", symbol=self.current_ast_node
        )
        return None
    ...
    def _visit_Module(self, expr):
        ...
        if not self.is_stub_file:
            ...
            pyccel_stage.set_stage("syntactic")
            funcs_to_insert = [
                f[0] if len(f) == 1 else Interface(n, f) for n, f in funcs.items()
            ]
            pyccel_stage.set_stage("semantic")
        ...
        if expr.program:
            ...
            self.scope = mod_scope.new_child_scope(
                prog_name,
        ...
        funcs_to_visit = [
            f
            for func in self.scope.functions.values()
        ...
        if not errors.has_errors():
            assert all(
                f.is_semantic or f.is_inline for f in self.scope.functions.values()
            )
        ...
        if self.is_stub_file:
            ...
            init_var = Variable(
                PythonNativeBool(),
                self.scope.get_new_name("initialised"),
                is_private=True,
                is_temp=True,
            )
            ...
            init_scope = self.create_new_function_scope(
                syntactic_init_func_name, init_func_name
            )
            ...
            unused_symbols = [
                n
                for n in self.scope.local_used_symbols
        ...
        if self.is_stub_file:
            ...
            pyccelised_imports = [
                imp
                for imp_name, imp in self.scope.imports["imports"].items()
                if imp_name in self.d_parsers
            ]

            import_frees = [
                self.d_parsers[imp.source].semantic_parser.ast.free_func
                for imp in pyccelised_imports
                if imp.source in self.d_parsers
            ]
            import_frees = [
                next(t.object for t in imp.target if t.name == f.name)
            ...
            if deallocs or import_frees:
                ...
                free_func_body = If(
                    IfSection(
                        ...
                        import_free_calls
                        + deallocs
                        + [Assign(init_var, LiteralFalse())],
                    )
                )
                # Ensure that the function is correctly defined within the namespaces
                scope = self.create_new_function_scope(
                    syntactic_free_func_name, free_func_name
                )
        ...
        if expr.program:
            ...
            container["imports"][mod_name] = Import(
                self.scope.get_python_name(mod_name), mod
            )
            ...
            used_datatypes = {
                v.class_type for v in program_body.get_attribute_nodes(Variable)
            }
            ...
            for i in self.scope.imports["imports"].values():
                ...
                for t in i.target:
                    ...
                    if local_t and (
                        program_body.is_user_of(local_t, excluded_nodes=(FunctionDef,))
                        or (
                            isinstance(local_t, ClassDef)
                            and local_t.class_type in used_datatypes
                        )
                    ):
                        target.append(t)
                if target:
    ...
    def _visit_PythonTuple(self, expr):
        ...
        if expr.get_user_nodes(
            Return, (IndexedElement, FunctionCall, PyccelFunction, PyccelOperator)
        ):
            func = expr.get_user_nodes(FunctionDef)[0]
            n_returns = set(
                r.n_explicit_results for r in func.get_attribute_nodes(Return)
            )
    ...
    def _visit_PythonList(self, expr):
        ...
        try:
            ...
            errors.report(
                PYCCEL_RESTRICTION_INHOMOG_LIST, symbol=expr, severity="fatal"
            )
        return expr
    ...
    def _visit_PythonSet(self, expr):
        ...
        try:
            ...
            message = str(e)
            errors.report(message, symbol=expr, severity="fatal")
        return expr
    ...
    def _visit_PythonDict(self, expr):
        ...
        try:
            ...
            errors.report(str(e), symbol=expr, severity="fatal")
        return expr
    ...
    def _visit_FunctionCallArgument(self, expr):
        ...
        def generate_and_assign_temp_var():
            ...
            tmp_var = self.scope.get_new_name()
            syntactic_assign = Assign(
                tmp_var, expr.value, python_ast=expr.value.python_ast
            )
            pyccel_stage.set_stage("semantic")
        ...
        elif isinstance(value, FunctionCall) and isinstance(
            value.class_type, CustomDataType
        ):
    ...
    def _visit_UnionTypeAnnotation(self, expr):
        annotations = [
            self._visit(syntax_type_annot) for syntax_type_annot in expr.type_list
        ]
        types = [
            t
            for a in annotations
            for t in (a.type_list if isinstance(a, UnionTypeAnnotation) else [a])
        ]
        ...
        for v in arg:
            if isinstance(v, Variable):
                ...
                if isinstance(dtype, InhomogeneousTupleType):
                    ...
                    errors.report(
                        "Inhomogeneous tuples are not yet supported as arguments",
                        severity="error",
                        symbol=expr,
                    )
                ...
                args.append(
                    FunctionDefArgument(
                        v.clone(v.name, is_optional=is_optional, is_argument=True),
                        posonly=posonly,
                        kwonly=kwonly,
                        bound_argument=bound_argument,
                        value=value,
        ...
        for b in expr.body:
            ...
            if isinstance(line, CodeBlock):
                ...
                if isinstance(var, DottedVariable):
                    ...
                    cls_def.scope.insert_variable(
                        var.clone(var.name, new_class=Variable)
                    )
                else:
    ...
    def _visit_OmpAnnotatedComment(self, expr):
        ...
        combined_loop = expr.combined and (
            "for" in expr.combined
            or "distribute" in expr.combined
            or "taskloop" in expr.combined
        )

        if (
            isinstance(expr, (OMP_Sections_Construct, OMP_Single_Construct))
            and expr.has_nowait
        ):
        ...
        if (
            ...
            index += 1
            while index < len(code.body) and isinstance(
                code.body[index], (Comment, CommentBlock, Pass)
            ):
                index += 1
            ...
            if index < len(code.body) and isinstance(code.body[index], For):
                ...
                code.body[index].end_annotation = (
                    " ".join(e for e in end_expr if e) + "\n"
                )
            else:
                type_name = type(expr).__name__
    ...
    def _visit_Omp_End_Clause(self, expr):
        ...
        if end_loop:
            errors.report(
                "For loops do not require an end clause. This clause is ignored",
                severity="warning",
                symbol=expr,
            )
            return EmptyNode()
    ...
    def _visit_IndexedElement(self, expr):
        ...
        if isinstance(
            var, (PyccelFunctionDef, VariableTypeAnnotation, UnionTypeAnnotation)
        ):
        ...
        if isinstance(class_type, (NumpyNDArrayType, HomogeneousListType, TupleType)):
            ...
            if len(args) == 1 and isinstance(
                ...
                isinstance(getattr(a, "class_type", None), TupleType) for a in args
            ):
                n_exprs = None
                for a in args:
                    if getattr(a, "shape", None) and isinstance(
                        a.shape[0], LiteralInteger
                    ):
                        a_len = a.shape[0]
                        if n_exprs:
                ...
                if n_exprs is not None:
                    new_expr_args = [
                        [a[i] if hasattr(a, "__getitem__") else a for a in args]
            ...
            if method:
                ...
                raise errors.report(
                    f"No __getitem__ found for type {class_type}",
                    severity="fatal",
                    symbol=expr,
                )
    ...
    def _visit_AnnotatedPyccelSymbol(self, expr):
        ...
        if var is not None and not any(
            isinstance(n, FunctionDefResult) for n in var.get_all_user_nodes()
        ):
            errors.report(
        ...
        if expr.annotation is None:
            errors.report(MISSING_TYPE_ANNOTATIONS, symbol=expr, severity="fatal")
            ...
            if isinstance(t, FunctionTypeAnnotation):
                ...
                if t.result.var:
                    results = FunctionDefResult(
                        t.result.var.clone(t.result.var.name, is_argument=False),
                ...
                if "memory_handling" not in var_kwargs:
                    var_kwargs["memory_handling"] = (
                        array_memory_handling if class_type.rank > 0 else "stack"
                    )
                v = var_class(
                    class_type,
                    name,
                ...
                if isinstance(class_type, InhomogeneousTupleType):
                    for i, t in enumerate(class_type):
                        pyccel_stage.set_stage("syntactic")
                        syntactic_elem = AnnotatedPyccelSymbol(
                            self.scope.get_new_name(f"{name}_{i}"),
        ...
        if var is not None:
            ...
            if len(possible_args) != 1 or new_var.class_type != var.class_type:
                errors.report(
                    f"Variable was declared as the result of the function {self.current_function_name} but is now declared with a different type",
                    symbol=expr,
                    severity="error",
                )
    ...
    def _visit_SyntacticTypeAnnotation(self, expr):
        ...
        if (
            isinstance(visited_dtype, UnionTypeAnnotation)
            and len(visited_dtype.type_list) == 1
        ):
        ...
        if isinstance(visited_dtype, PyccelFunctionDef):
            ...
            try:
                ...
                errors.report(
                    f"Unrecognised datatype {dtype_cls}", severity="fatal", symbol=expr
                )
            ...
            if order and order != visited_dtype.class_type.order:
                visited_dtype = VariableTypeAnnotation(
                    visited_dtype.class_type.swap_order()
                )
            ...
            raise errors.report(
                PYCCEL_RESTRICTION_TODO + " Could not deduce type information",
                severity="fatal",
                symbol=expr,
            )
    ...
    def _visit_DottedName(self, expr):
        ...
        lhs = expr.name[0] if len(expr.name) == 2 else DottedName(*expr.name[:-1])
        rhs = expr.name[-1]
        ...
        if isinstance(visited_lhs, FunctionCall):
            ...
            if len(results) != 1:
                errors.report(
                    "Cannot get attribute of function call with multiple returns",
                    symbol=expr,
                    severity="fatal",
                )
            first = results.var
        ...
        if isinstance(first, Module):
            ...
            if rhs_name in first:
                ...
                if isinstance(rhs_obj, PyccelFunctionDef):
                    assert (
                        new_name not in scope.imports["functions"]
                        or scope.imports["functions"][new_name] == rhs_obj
                    )
                    scope.imports["functions"][new_name] = rhs_obj
                elif isinstance(rhs, FunctionCall):
                    if new_name not in scope.imports["functions"]:
                        m = rhs_obj.get_direct_user_nodes(
                ...
                errors.report(
                    UNDEFINED_IMPORT_OBJECT.format(rhs_name, str(lhs)),
                    symbol=expr,
                    severity="fatal",
                )
        ...
        # look for a class attribute / property
            ...
            # class property?
                ...
                return self._handle_function(
                    expr, method, [FunctionCallArgument(visited_lhs)], is_method=True
                )

        # did something go wrong?
        return errors.report(
            f"Attribute {rhs_name} not found",
            bounding_box=(
                self.current_ast_node.lineno,
                self.current_ast_node.col_offset,
            ),
    ...
    def _visit_PyccelAdd(self, expr):
        ...
        if isinstance(arg0.class_type, (TupleType, HomogeneousListType)):
            arg1 = args[1]
            is_homogeneous = (
                not isinstance(arg0.class_type, InhomogeneousTupleType)
                and arg0.class_type == arg1.class_type
            )
            if is_homogeneous:
                return Concatenate(*args)
            else:
                if not (
                    isinstance(arg0.shape[0], (LiteralInteger, int))
                    and isinstance(arg1.shape[0], (LiteralInteger, int))
                ):
                    errors.report(
                        "Can't create an inhomogeneous object from objects of unknown size",
                        severity="fatal",
                        symbol=expr,
                    )

                tuple_args = [self.scope.collect_tuple_element(v) for v in arg0] + [
                    self.scope.collect_tuple_element(v) for v in arg1
                ]
    ...
    def _visit_PyccelPow(self, expr):
        ...
        if isinstance(exponent, LiteralInteger):
            ...
            exp = exponent.args[0]
            # Handle (1/2)
            if isinstance(exp, PyccelDiv) and all(
                isinstance(a, Literal) for a in exp.args
            ):
        ...
        if isinstance(base, (Literal, Variable)) and exp_val == 2:
            ...
            if isinstance(
                base.class_type.primitive_type, PrimitiveComplexType
            ) or isinstance(exponent.class_type.primitive_type, PrimitiveComplexType):
    ...
    def _visit_PyccelIn(self, expr):
        ...
        if isinstance(
            container_type, (DictType, HomogeneousSetType, HomogeneousListType)
        ):
            element_type = (
                container_type.key_type
                if isinstance(container_type, DictType)
                else container_type.element_type
            )
        ...
        if contains_method:
            return self._handle_function(
                expr,
                contains_method,
                [FunctionCallArgument(container), FunctionCallArgument(element)],
            )
        else:
            raise errors.report(
                f"In operator is not yet implemented for type {container_type}",
                severity="fatal",
                symbol=expr,
            )
    ...
    def _visit_FunctionCall(self, expr):
        ...
        if (isinstance(func, FunctionDef) and func.scope) or isinstance(
            func, Interface
        ):
            scope = (
                func.scope if isinstance(func, FunctionDef) else func.functions[0].scope
            )
            args = [
                (
                    a
                    if a.keyword is None
                    else FunctionCallArgument(
                        a.value, scope.local_used_symbols.get(a.keyword, a.keyword)
                    )
                )
                for a in args
            ]
            ...
            if not func.is_semantic:
                ...
                func_args = [
                    FunctionDefArgument(
                        AnnotatedPyccelSymbol(
                            scope.get_expected_name(a.var.name), a.annotation
                        ),
                        annotation=a.annotation,
                        value=a.value,
                        posonly=a.is_posonly,
                        kwonly=a.is_kwonly,
                        bound_argument=a.bound_argument,
        ...
        if self.scope.find(name, "cls_constructs"):
            ...
            if init_method is not None:
                ...
                if isinstance(lhs, AnnotatedPyccelSymbol):
                    ...
                    if (
                        len(annotation.type_list) != 1
                        or annotation.type_list[0].class_type
                        != init_method.arguments[0].var.class_type
                    ):
                        errors.report(
                            f"Unexpected type annotation in creation of {cls_def.name}",
                            symbol=annotation,
                            severity="error",
                        )
                    lhs = lhs.name

                cls_variable = self._assign_lhs_variable(
                    lhs,
                    d_var,
                ...
                for a, f_a in zip(new_expr.args, init_method.arguments):
                    if f_a.persistent_target:
                        ...
                        if isinstance(val, Variable):
                            a.value.is_target = True
                            self._indicate_pointer_target(
                                cls_variable, a.value, expr.get_user_nodes(Assign)[0]
                            )
                        else:
                            errors.report(
                                f"{val} cannot be passed to class constructor call as target. Please create a temporary variable.",
                                severity="error",
                                symbol=expr,
                            )
            else:
            ...
            if func is None and name in self._context_dict:
                ...
                if isinstance(env_var, FunctionDef):
                    ...
                    recognised_mod = (mod_name is not None) and recognised_source(
                        mod_name
                    )
    ...
    def _visit_Assign(self, expr):
        ...
        if isinstance(lhs, AnnotatedPyccelSymbol):
            ...
            if len(semantic_lhs) != 1:
                errors.report(
                    "Cannot declare variable with multiple types",
                    symbol=expr,
                    severity="error",
                )
        ...
        if isinstance(rhs, (PythonTuple, PythonList)):
            ...
            if isinstance(lhs, PythonTuple):
                ...
                pyccel_stage.set_stage("syntactic")
                unsaved_vars = set(
                    rhs.get_attribute_nodes(
                        (PyccelSymbol, DottedName, IndexedElement),
                ...
                modified_vars = set(
                    lhs.get_attribute_nodes((PyccelSymbol, DottedName, IndexedElement))
                )
                used_vars = set(
                    rhs.get_attribute_nodes(
                        (PyccelSymbol, DottedName, IndexedElement),
                ...
                all_indexed_are_simple = all(
                    all(
                        isinstance(idx, (PyccelSymbol, DottedName, Literal))
                        for idx in elem.indices
                    )
                    for elem in modified_vars
                    if isinstance(elem, IndexedElement)
                )
                if not trivial_assign and (
                    used_vars.intersection(modified_vars).difference(unsaved_vars)
                    or not all_indexed_are_simple
                ):
                    errors.report(
                ...
                for i, l in enumerate(lhs):
                    ...
                    unsaved_vars = set(
                        tmp_rhs_tuple.get_attribute_nodes(
                            (PyccelSymbol, DottedName, IndexedElement),
                            excluded_nodes=(FunctionDef,),
                        )
                    )
                    ...
                    if l in unsaved_vars:
                        ...
                        if isinstance(l, IndexedElement):
                            ...
                            l_list = [
                                r
                                for r in rhs.get_attribute_nodes(IndexedElement)
                                if r == l
                            ]
                        else:
                            l_list = [l]
                ...
                if semantic_lhs and isinstance(
                    semantic_lhs.class_type, InhomogeneousTupleType
                ):
                    pyccel_stage.set_stage("syntactic")
                    syntactic_assign_elems = [
                        Assign(IndexedElement(lhs, i), r, python_ast=expr.python_ast)
            ...
            if assign_elems is not None:
                return CodeBlock(
                    [
                        l
                        for a in assign_elems
                        for l in (a.body if isinstance(a, CodeBlock) else [a])
                    ]
                )
        ...
        if isinstance(rhs, (PythonMap, PythonZip, PythonEnumerate, PythonRange)):
            errors.report(
                f"{type(rhs)} cannot be saved to variables",
                symbol=expr,
                severity="fatal",
            )
        ...
        if isinstance(rhs, NumpyResultType):
            errors.report(
                "Cannot assign a datatype to a variable.", symbol=expr, severity="error"
            )
        ...
        if isinstance(rhs, FunctionCall):
            ...
            if results:
                ...
                raise errors.report(
                    "Cannot assign result of a function without a return",
                    severity="fatal",
                    symbol=expr,
                )

            if isinstance(results.class_type, NumpyNDArrayType) and isinstance(
                lhs, IndexedElement
            ):
                temp = self.scope.get_new_name()
                semantic_temp = self._assign_lhs_variable(
                    temp, d_var, rhs, new_expressions
                )
                new_expressions.append(Assign(semantic_temp, rhs))
                rhs = semantic_temp
                errors.report(
                    (
                        f"Saving the result of the function {func.name} to a slice requires unnecessary "
            ...
            if d_var["memory_handling"] == "alias" and not isinstance(
                lhs, IndexedElement
            ):
        ...
        if isinstance(lhs, (PyccelSymbol, DottedName)):
            if isinstance(d_var, list):
                if len(d_var) == 1:
                    ...
                    errors.report(
                        WRONG_NUMBER_OUTPUT_ARGS, symbol=expr, severity="error"
                    )
                    return None
            ...
            if isinstance(rhs, PythonTuple) and isinstance(
                lhs.class_type, InhomogeneousTupleType
            ):
                for li, ri in zip(lhs, rhs):
                    li_var = self.scope.collect_tuple_element(li)
                    if li_var == ri:
                        new_expressions = [
                            n for n in new_expressions if not n.is_user_of(li_var)
                        ]
        ...
        # Handle assignment to multiple variables
            ...
            if isinstance(rhs, FunctionCall):
                ...
                for i, (l, r) in enumerate(zip(lhs, rhs.funcdef.results.var)):
                    ...
                    new_lhs.append(
                        self._assign_lhs_variable(
                            l, d, r, new_expressions, heap_mem_in_multirets=r.rank > 0
                        )
                    )
        ...
        while unravelling:
            ...
            for l, r in zip(lhs, rhs):
                ...
                if (
                    isinstance(l.class_type, InhomogeneousTupleType)
                    or isinstance(l, PythonTuple)
                ) and not isinstance(r, (FunctionCall, PyccelFunction)):
                    new_lhs.extend(self.scope.collect_tuple_element(v) for v in l)
                    new_rhs.extend(
                        self.scope.collect_tuple_element(r[i])
                        for i in range(l.shape[0])
                    )
                    # Repeat step to handle tuples of tuples of etc.
                    unravelling = True
                elif isinstance(l, Variable) and isinstance(
                    l.class_type, InhomogeneousTupleType
                ):
                    new_lhs.append(
                        PythonTuple(*self.scope.collect_all_tuple_elements(l))
                    )
                    new_rhs.append(r)
                    # Repeat step to handle tuples of tuples of etc.
                    unravelling = True
                elif isinstance(l, Variable) and isinstance(
                    r.class_type, InhomogeneousTupleType
                ):
        ...
        for l, r in zip(lhs, rhs):
            if isinstance(l, PythonTuple):
                ...
                if isinstance(l.class_type, FinalType) and (
                    not isinstance(expr.lhs, AnnotatedPyccelSymbol)
                    or any(
                        not isinstance(u, (Allocate, PyccelArrayShapeElement))
                        for u in l.get_all_user_nodes()
                    )
                ):
        ...
        if expr.lhs == "__all__":
            ...
            if isinstance(lhs[0].class_type, HomogeneousListType):
                ...
                try:
                    error_info_map = errors.error_info_map[
                        os.path.basename(errors.target)
                    ]
    ...
    def _visit_AugAssign(self, expr):
        ...
        if test_node:
            ...
            lhs = self._assign_lhs_variable(
                expr.lhs,
                self._infer_type(operator_node),
                test_node,
                new_expressions,
                is_augassign=True,
            )
    ...
    def _visit_For(self, expr):
        ...
        target, iterable = self._get_for_iterators(
            expr.iterable, expr.target, new_expr, expr
        )
        ...
        if isinstance(iterable, Product):
            ...
            for t, i, r, s in zip(
                target[::-1],
                iterable.loop_counters[::-1],
                iterable.get_python_iterable_item()[::-1],
                scopes[::-1],
            ):
    ...
    def _visit_FunctionalFor(self, expr):
        ...
        def handle_int_loop_variable(var_name, var_scope):
            ...
            var = self._create_variable(
                var_name, PythonNativeInt(), None, {}, insertion_scope=var_scope
            )
            return var
        ...
        def handle_iterable_variable(var_name, element, var_scope):
            ...
            var = self._create_variable(
                var_name, class_type, None, dvar, insertion_scope=var_scope
            )
            return var
        ...
        while isinstance(body, (For, If)):
            ...
            if isinstance(body, If):
                body = (
                    None
                    if not body.blocks[0].body.body
                    else body.blocks[0].body.body[0]
                )
                continue
            ...
            if isinstance(a, PythonRange):
                ...
                if isinstance(iter_var, IndexedElement):
                    ...
                    if getattr(base, "allows_negative_indexes", False) and any(
                        not is_literal_integer(s) for s in (start, stop, step)
                    ):
            ...
            for var in variables:
                ...
                if var.name == expr.lhs:
                    errors.report(
                        f"Variable {var} has the same name as the left hand side",
                        symbol=expr,
                        severity="fatal",
                    )
                if existing_var or var.name == expr.lhs:
                    if self._infer_type(existing_var)["class_type"] != var.class_type:
                        errors.report(
                            f"Variable {var} already exists with different type",
                            symbol=expr,
                            severity="fatal",
                        )
                else:
        ...
        for i in reversed(range(len(dims))):
            ...
            start_idx = [-1] + [
                sp_indices.index(a) for a in start.atoms(sp_Symbol) if a in sp_indices
            ]
            stop_idx = [-1] + [
                sp_indices.index(a) for a in stop.atoms(sp_Symbol) if a in sp_indices
            ]
            start_idx.sort()
            stop_idx.sort()
            ...
            if isinstance(min_size, sp_Integer) and min_size < 0:
                errors.report(
                    PYCCEL_RESTRICTION_LIST_COMPREHENSION_LIMITS.format(indices[i]),
                    bounding_box=(
                        self.current_ast_node.lineno,
                        self.current_ast_node.col_offset,
                    ),
        ...
        try:
            ...
            errors.report(
                PYCCEL_RESTRICTION_LIST_COMPREHENSION_SIZE + f"\n Deduced size : {dim}",
                symbol=expr,
                severity="fatal",
            )
        ...
        if isinstance(target_type_name, DottedName):
            lhs = (
                target_type_name.name[0]
                if len(target_type_name.name) == 2
            ...
            if conversion_func is None:
                if target_type_name in builtin_functions_dict:
                    conversion_func = PyccelFunctionDef(
                        target_type_name, builtin_functions_dict[target_type_name]
                    )
        if conversion_func is None:
            errors.report(
                "Unrecognised output type from functional for.\n"
                + PYCCEL_RESTRICTION_TODO,
                symbol=expr,
                severity="fatal",
            )
        if target_type_name != "list" and any(
            cond is not None for cond in expr.conditions
        ):
            errors.report(
                "Cannot handle if statements in list comprehensions if lhs is a numpy array.\
        ...
        try:
            ...
            if class_type.rank > 0:
                errors.report(
                    "ND comprehension expressions cannot be saved directly to an array yet.\n"
                    + PYCCEL_RESTRICTION_TODO,
                    symbol=expr,
                    severity="fatal",
                )

            class_type = type_container[conversion_func.cls_name].get_new(
                numpy_process_dtype(class_type), rank=1, order=None
            )
        ...
        if (
            isinstance(target_conversion_func, PyccelFunctionDef)
            and target_conversion_func.cls_name is NumpyArray
        ) or isinstance(lhs_alloc, EmptyNode):
            ...
            for operation in array_ops:
                operation.substitute(
                    old_index, new_index, is_equivalent=lambda x, y: x is y
                )
            assign = array_ops[0]
            ...
            errors.report(
                "Unrecognised target for functional for.\n" + PYCCEL_RESTRICTION_TODO,
                symbol=expr,
                severity="fatal",
            )
        ...
        if expr.loops[-1].body.body:
            for operation in operations:
                expr.loops[-1].body.body[0].blocks[0].body.insert2body(
            ...
            for non_conditional in non_conditional_list:
                ...
                if len(var_use) > 1:
                    ...
                    if isinstance(cond, PyccelAnd) and non_conditional in cond.args:
                        remaining_cond = (
                            PyccelAnd(
                                *[a for a in cond.args if a is not non_conditional]
                            )
                            if len(cond.args) > 2
                            else next(a for a in cond.args if a is not non_conditional)
                        )
                        remaining_cond.set_current_ast(cond.python_ast)
                        cond_var = self.scope.get_temporary_variable(
                            PythonNativeBool(), self.scope.get_new_name("condition")
                        )
                        treated_remaining_cond, body = treat_condition(
                            remaining_cond, body
                        )
                        if is_last_block:
                            ...
                            self._additional_exprs[-1].append(
                                If(
                                    IfSection(
                                        non_conditional,
                                        [Assign(cond_var, treated_remaining_cond)],
                                    ),
                                    IfSection(
                                        LiteralTrue(),
                                        [Assign(cond_var, LiteralFalse())],
                                    ),
                                )
                            )
                            cond = cond_var
                        ...
                        errors.report(
                            "Cannot evaluate condition. Checking if a variable is present must be done before using the variable",
                            severity="error",
                            symbol=cond,
                        )
            return cond, body
    ...
    def _visit_If(self, expr):
        ...
        for v in variables:
            all_shapes_set = all(
                v in branch_shapes.keys() for branch_shapes in var_shapes
            )
            if all_shapes_set:
                shape_branch1 = var_shapes[0][v]
                same_shapes = all(
                    shape_branch1 == branch_shapes[v]
    ...
    def _visit_Return(self, expr):
        ...
        if (
            original_name.startswith("__i")
            and ("__" + original_name[3:]) in magic_method_map.values()
        ):
            valid_return = (
                isinstance(expr.expr, PyccelSymbol)
                and expr.stmt is None
            ...
            if valid_return:
                ...
                errors.report(
                    "Increment functions must return the class instance",
                    severity="fatal",
                    symbol=expr,
                )
        ...
        code = assigns + [
            Deallocate(i) for i in self._allocs[-1] if i not in results_vars
        ]
        if results is Nil():
            results = None
        if code:
    ...
    def _visit_FunctionDef(self, expr):
        ...
        if expr.get_direct_user_nodes(lambda u: isinstance(u, CodeBlock)):
            errors.report(
                "Functions can only be declared in modules, classes or inside other functions.",
                symbol=expr,
                severity="error",
            )
        ...
        if any(a.annotation is None for a in expr.arguments):
            ...
            errors.report(
                msg,
                symbol=[a for a in expr.arguments if a.annotation is None],
                severity="error",
                bounding_box=(
                    self.current_ast_node.lineno,
                    self.current_ast_node.col_offset,
        ...
        if any(a.annotation is None for a in expr.arguments):
            errors.report(MISSING_TYPE_ANNOTATIONS, symbol=expr, severity="error")
            return EmptyNode()
        ...
        possible_combinations = list(
            product(*[t.type_list for t in used_type_vars.values()])
        )
        ...
        for interface_idx, (arguments, type_var_idx) in enumerate(
            ...
            for a in arguments:
                ...
                if isinstance(a_var, FunctionAddress):
                    ...
                    self.scope.insert_variable(
                        a_var, expr.scope.get_python_name(a.name)
                    )

            if arguments and arguments[0].bound_argument:
                if arguments[0].var.cls_base is not bound_class:
                    errors.report(
                        "Class method self argument does not have the expected type",
                        severity="error",
                        symbol=arguments[0],
                    )
            ...
            sub_funcs = [
                i
                for i in self.scope.functions.values()
                if not i.is_header
                and not isinstance(i, (InlineFunctionDef, FunctionAddress))
                and not i.is_semantic
            ]
            for i in sub_funcs:
                self._visit(i)
            ...
            if func_.is_recursive and not is_inline:
                ...
                errors.report(
                    "Pyccel does not support an inlined recursive function",
                    symbol=expr,
                    severity="fatal",
                )
            ...
            sub_funcs = [
                i
                for i in self.scope.functions.values()
                if not i.is_header and not isinstance(i, FunctionAddress)
            ]
            ...
            for r in results_vars:
                ...
                if r.is_alias:
                    ...
                    if temp_targets:
                        errors.report(
                            UNSUPPORTED_POINTER_RETURN_VALUE, symbol=r, severity="error"
                        )
                    else:
                        result_pointer_map[r] = [
                            next(i for i, a in enumerate(arguments) if a.var == target)
                            for target, _ in t
                        ]
            ...
            for a in arguments:
                ...
                if var:
                    optional_inits.append(
                        If(IfSection(PyccelIsNot(a.var, Nil()), [Assign(var, a.var)]))
                    )
            ...
            if is_inline:
                ...
                global_funcs = [
                    f
                    for f in body.get_attribute_nodes(FunctionDef)
                ...
                cls = FunctionDef
            func = cls(name, arguments, body, results, **func_kwargs)
            if not is_recursive:
        ...
        if len(new_semantic_funcs) == 1 and not is_interface:
            ...
            new_semantic_funcs = Interface(
                interface_name, new_semantic_funcs, syntactic_node=expr
            )
            if expr.python_ast:
    ...
    def _visit_InlineFunctionCall(self, expr, function_call_args, function_call):
        ...
        assign = function_call.get_direct_user_nodes(
            lambda a: isinstance(a, Assign) and not isinstance(a, AugAssign)
        )
        ...
        if is_imported:
            mod_name = expr.get_direct_user_nodes(lambda m: isinstance(m, Module))[
                0
            ].name
            ...
            while to_examine:
                ...
                for ex in to_examine:
                    if isinstance(ex, DottedName) and isinstance(
                        ex.name[-1], FunctionCall
                    ):
                        global_symbols.add(ex.name[0])
                        remaining = ex.name[-1].get_attribute_nodes(
                            (PyccelSymbol, DottedName)
                        )
                        remaining.remove(ex.name[-1].funcdef)
                        global_symbols.update(
                            s for s in remaining if not isinstance(s, DottedName)
                        )
                        remaining_to_examine.extend(
                            s for s in remaining if isinstance(s, DottedName)
                        )
                    else:
                        symbols = ex.get_attribute_nodes((PyccelSymbol, DottedName))
                        global_symbols.update(
                            s for s in symbols if not isinstance(s, DottedName)
                        )
                        remaining_to_examine.extend(
                            s for s in symbols if isinstance(s, DottedName)
                        )
            ...
            for v in global_symbols:
                ...
                if mod.scope.find(v):
                    ...
                    if isinstance(imported_obj, Module):
                        ...
                        mod_import = (
                            source if source == new_v else AsName(source, new_v)
                        )
                        imports.append(Import(mod_import))
                    else:
                        if imported_obj:
                            import_mod_name = imported_obj.get_direct_user_nodes(
                                lambda m: isinstance(m, Module)
                            )[0].name
                        if self.scope.symbol_in_use(v):
                            new_v = self.scope.get_new_name(
                                self.scope.get_expected_name(v)
                            )
                            replace_map[v] = new_v
                            global_scope_import_targets.setdefault(
                                import_mod_name, []
                            ).append(AsName(v, new_v))
                        else:
                            global_scope_import_targets.setdefault(
                                import_mod_name, []
                            ).append(v)
        ...
        func_args = [
            a.name if isinstance(a, AnnotatedPyccelSymbol) else a for a in func_args
        ]
        ...
        positional_call_args = [
            a.value for a in function_call_args if not a.has_keyword
        ]
        for func_a, call_a in zip(func_args, positional_call_args):
            if (
                isinstance(call_a, Variable)
                and not isinstance(call_a, DottedVariable)
                and func_a == self.scope.get_expected_name(call_a.name)
            ):
        ...
        for func_a, func_a_name in zip(expr.arguments[nargs:], func_args[nargs:]):
            call_a = kw_call_args.get(
                func_a_name,
                getattr(func_a.default_call_arg, "value", func_a.default_call_arg),
            )
            if isinstance(
                call_a, Variable
            ) and func_a_name == self.scope.get_expected_name(call_a.name):
                # If call argument is a variable with the same name as the target function
                # argument then there is no need to rename
        ...
        replace_return = [
            (
                Assign(lhs, r.expr, python_ast=r.python_ast)
                if not isinstance(r.expr, PyccelSymbol)
                or not isinstance(lhs, PyccelSymbol)
                else EmptyNode()
            )
            for r in returns
        ]
        ...
        new_imports = [
            Import(m_name, targets)
            for m_name, targets in global_scope_import_targets.items()
        ]
        ...
        if expr.functions:
            errors.report(
                "Functions in inline functions are not supported",
                severity="error",
                symbol=expr,
            )
        ...
        if assign:
            ...
            if (
                len(returns) == 1
                and (
                    isinstance(semantic_lhs.class_type, FixedSizeNumericType)
                    or semantic_lhs.is_alias
                )
                and isinstance(assign, (Assign, AliasAssign))
                and semantic_lhs.name == lhs
            ):
    ...
    def _visit_PythonPrint(self, expr):
        ...
        def is_symbolic(var):
            return isinstance(var, Variable) and isinstance(var.dtype, SymbolicType)
        ...
        if any(isinstance(a.value.class_type, InhomogeneousTupleType) for a in args):
            ...
            for a in args:
                ...
                if isinstance(val.class_type, InhomogeneousTupleType):
                    ...
                    new_args.append(
                        FunctionCallArgument(
                            self.create_tuple_of_inhomogeneous_elements(val)
                        )
                    )
                else:
                    new_args.append(a)
    ...
    def _visit_ClassDef(self, expr):
        ...
        if expr.get_direct_user_nodes(lambda u: isinstance(u, CodeBlock)):
            errors.report(
                "Classes can only be declared in modules.",
                symbol=expr,
                severity="error",
            )
        ...
        for a in attribute_annotations:
            if len(a) != 1:
                errors.report(
                    f"Couldn't determine type of {a}", severity="error", symbol=a
                )
            else:
                v = a[0]
    ...
    def _visit_PyccelIs(self, expr):
        ...
        if isinstance(var1.dtype, PythonNativeBool) and isinstance(
            var2.dtype, PythonNativeBool
        ):
            return IsClass(var1, var2)

        if isinstance(var1.class_type, (StringType, FixedSizeNumericType)):
            errors.report(
                PYCCEL_RESTRICTION_PRIMITIVE_IMMUTABLE, symbol=expr, severity="error"
            )
            return IsClass(var1, var2)

        errors.report(PYCCEL_RESTRICTION_IS_ISNOT, symbol=expr, severity="error")
        return IsClass(var1, var2)
    ...
    def _visit_Import(self, expr):
        ...
        if expr.get_direct_user_nodes(lambda u: isinstance(u, CodeBlock)):
            errors.report(
                "Imports can only be used in modules or inside functions.",
                symbol=expr,
                severity="error",
            )
        ...
        if source in pyccel_builtin_import_registry:
            ...
            def _insert_obj(location, target, obj):
                ...
                if obj is F:
                    errors.report(
                        FOUND_DUPLICATED_IMPORT, symbol=target, severity="warning"
                    )
                elif F is None or isinstance(F, dict):
                    container[location][target] = obj
                else:
                    errors.report(
                        IMPORTING_EXISTING_IDENTIFIED, symbol=expr, severity="fatal"
                    )
            ...
            if expr.target:
                for t in expr.target:
                    ...
                    if t_name not in pyccel_builtin_import_registry[source]:
                        errors.report(
                            f"Function '{t}' from module '{source}' is not currently supported by pyccel",
                            symbol=expr,
                            severity="error",
                        )
            ...
            self.insert_import(
                source, [AsName(v, n) for n, v in imports], source_target
            )

        elif recognised_source(source):
            errors.report(
                f"Module {source} is not currently supported by pyccel",
                symbol=expr,
                severity="error",
            )
        else:
            ...
            import_init = (
                p.semantic_parser.ast.init_func
                if source_target not in container["imports"]
                else None
            )
            import_free = (
                p.semantic_parser.ast.free_func
                if source_target not in container["imports"]
                else None
            )
            if expr.target:
                targets = {
                    i.local_alias if isinstance(i, AsName) else i: None
                        ...
                        if n in d_son:
                            ...
                            if entry == "functions":
                                if t in container[entry]:
                                    ...
                                    assert (
                                        e.get_direct_user_nodes(
                                            lambda x: isinstance(x, Module)
                                        )[0]
                                        is existing_e.get_direct_user_nodes(
                                            lambda x: isinstance(x, Module)
                                        )[0]
                                    )
                                else:
                                    assert t not in container[entry]
                                    container[entry][t] = e.clone(
                                        e.name, is_imported=True
                                    )
                                    m = e.get_direct_user_nodes(
                                        lambda x: isinstance(x, Module)
                                    )[0]
                ...
                if None in targets.values():
                    errors.report(
                        "Import target {} could not be found",
                        severity="warning",
                        symbol=expr,
                    )
    ...
    def _visit_With(self, expr):
        ...
        if not parent.is_with_construct:
            errors.report(UNDEFINED_WITH_ACCESS, symbol=expr, severity="fatal")
    ...
    def _visit_FunctionDefResult(self, expr):
        ...
        if isinstance(var, list):
            ...
            if n_types == 0:
                errors.report(
                    "Can't deduce type for function definition result.",
                    severity="fatal",
                    symbol=expr,
                )
            elif n_types != 1:
    ...
    def _build_ListExtend(self, expr, args, func):
        ...
        if isinstance(iterable, (PythonList, PythonTuple)):
            ...
            try:
                ...
                errors.report(msg, symbol=expr, severity="fatal")
            if not isinstance(
                list_variable.class_type.element_type,
                (StringType, FixedSizeNumericType),
            ):
                for a in added_list:
                    if not isinstance(
                        a, (PythonList, PythonSet, PythonTuple, NumpyNewArray)
                    ):
    ...
    def _build_CmathSqrt(self, func_call, func_call_args, func):
        ...
        if isinstance(arg.value, PyccelMul):
            ...
            is_abs = False
            if (
                isinstance(mul1, (NumpyConjugate, PythonConjugate))
                and mul1.internal_var is mul2
            ):
                is_abs = True
                abs_arg = mul2
            elif (
                isinstance(mul2, (NumpyConjugate, PythonConjugate))
                and mul1 is mul2.internal_var
            ):
                is_abs = True
                abs_arg = mul1
    ...
    def _build_CmathPolar(self, func_call, func_call_args, func):
        ...
        x = PythonReal(z)
        y = PythonImag(z)
        x_var = self.scope.get_temporary_variable(
            z, class_type=PythonNativeFloat(), is_argument=False
        )
        y_var = self.scope.get_temporary_variable(
            z, class_type=PythonNativeFloat(), is_argument=False
        )
    ...
    def _build_NumpyArray(self, expr, func_call_args, func):
        ...
        if isinstance(expr, DottedName):
            ...
            order = (
                func_call_args[0].value
                if func_call_args
                else func.argument_description["order"]
            )
        else:
        ...
        if not isinstance(arg, (PythonTuple, PythonList, Variable, IndexedElement)):
            errors.report(
                "Unexpected object passed to numpy.array", severity="fatal", symbol=expr
            )
        ...
        if isinstance(arg.class_type, InhomogeneousTupleType):
            is_homogeneous_tuple = isinstance(arg.dtype, FixedSizeNumericType) and len(
                set(a.rank for a in arg)
            )
            if not isinstance(arg, PythonTuple):
                arg = PythonTuple(*(self.scope.collect_tuple_element(a) for a in arg))

        if not (
            is_homogeneous_tuple or isinstance(arg.class_type, HomogeneousContainerType)
        ):
            errors.report(
                "Inhomogeneous type passed to numpy.array",
                severity="fatal",
                symbol=expr,
            )
        ...
        if not isinstance(order, (LiteralString, str)):
            errors.report(
                "Order must be specified with a literal string",
                severity="fatal",
                symbol=expr,
            )
        ...
        if ndmin is not None:
            if not isinstance(ndmin, (LiteralInteger, int)):
                errors.report(
                    "The minimum number of dimensions must be specified explicitly with an integer.",
                    severity="fatal",
                    symbol=expr,
                )
    ...
    def _build_SetUnion(self, expr, function_call_args, func):
        ...
        if all(a.class_type == class_type for a in args):
            ...
            if any(a.class_type.element_type != element_type for a in args):
                errors.report(
                    (
                        "Containers containing objects of a different type cannot be used as "
            ...
            body = [
                Assign(
                    lhs,
                    DottedName(syntactic_set_obj, FunctionCall("copy", ())),
                    python_ast=expr.python_ast,
                )
            ]
            update_calls = [
                DottedName(lhs, FunctionCall("update", (s_a,)))
                for s_a in syntactic_args
            ]
            for c in update_calls:
                c.set_current_ast(expr.python_ast)
            body += [
                Assign(PyccelSymbol("_", is_temp=True), c, python_ast=expr.python_ast)
                for c in update_calls
            ]
            pyccel_stage.set_stage("semantic")
    ...
    def _build_PythonLen(self, expr, function_call_args, func):
        ...
        if isinstance(arg, LiteralString):
            ...
            if magic_method:
                ...
                raise errors.report(
                    f"__len__ not implemented for type {class_type}",
                    severity="fatal",
                    symbol=expr,
                )
        elif arg.rank > 0:
    ...
    def _build_PythonSetFunction(self, expr, function_call_args, func):
        ...
        if isinstance(arg, (PythonList, PythonSet, PythonTuple)):
            ...
            body = []
            lhs_semantic_var = self._assign_lhs_variable(
                lhs, d_var, PythonSetFunction(arg), body
            )
            scope = self.create_new_loop_scope()
            targets, iterable = self._get_for_iterators(
                arg, self.scope.get_new_name(), body, expr
            )
            self.exit_loop_scope()
            body.append(
                For(
                    targets,
                    iterable,
                    [SetAdd(lhs_semantic_var, targets[0])],
                    scope=scope,
                )
            )
            if assigns:
    ...
    def _build_PythonIsInstance(self, expr, function_call_args, func):
        ...
        if isinstance(class_or_tuple, PythonTuple):
            ...
            return PyccelOr.make_simplified(
                *[
                    self._build_PythonIsInstance(
                        expr,
                        [obj_arg, FunctionCallArgument(class_type)],
                        PythonIsInstance,
                    )
            ...
            return PyccelOr.make_simplified(
                *[
                    self._build_PythonIsInstance(
                        expr,
                        [obj_arg, FunctionCallArgument(var_annot)],
                        PythonIsInstance,
                    )
            ...
            if isinstance(expected_type, type):
                ...
                errors.report(
                    f"Type {class_or_tuple} is not handled in isinstance call.",
                    severity="error",
                    symbol=expr,
                )
                return LiteralTrue()
    ...
    def _build_ListAppend(self, expr, args, func):
        ...
        try:
            ...
            errors.report(e, symbol=expr, severity="error")
        if not isinstance(
            append_arg.class_type, (StringType, FixedSizeNumericType)
        ) and not isinstance(
            append_arg, (PythonList, PythonSet, PythonTuple, NumpyNewArray)
        ):
    ...
    def _build_ListInsert(self, expr, args, func):
        ...
        try:
            ...
            errors.report(e, symbol=expr, severity="error")
        if not isinstance(
            new_elem.class_type, (StringType, FixedSizeNumericType)
        ) and not isinstance(
            new_elem, (PythonList, PythonSet, PythonTuple, NumpyNewArray)
        ):
        ...
        if cls_func_type.__name__.startswith("Numpy"):
            ...
            if out_arg:
                ...
                if isinstance(lhs, Variable) and lhs.is_optional:
                    errors.report(
                        f"Assuming that variable {lhs} is not None",
                        severity="warning",
                        symbol=expr,
                    )
                d_var = self._infer_type(rhs)
                ...
                new_expressions = []
                self._ensure_inferred_type_matches_existing(
                    class_type, d_var, lhs, True, new_expressions, rhs, False
                )
