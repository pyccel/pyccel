...
class OmpConstruct(BasicStmt):
    ...
    def __init__(self, omp_type, vclauses, **kwargs):
        ...
        if simd:
            ...
            txt += " " + simd
        if clauses:
            clause_expr, has_nowait = check_get_clauses(
                self, _valid_clauses, clauses, combined
            )
            txt += clause_expr
...
def check_get_clauses(name, valid_clauses, clauses, combined=None):
    ...
    for clause in clauses:
        if isinstance(clause, valid_clauses) and not (
            isinstance(clause, OmpCopyin) and isinstance(combined, OmpTargetParallel)
        ):
            if isinstance(clause, OmpNowait):
                if isinstance(
                    name, (OmpLoopConstruct, OmpSectionsConstruct, OmpSingleConstruct)
                ):
                    has_nowait = True
                else:
...
class OmpTaskLoopConstruct(OmpConstruct):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            OMP_TaskLoop_Construct,
            (_valid_taskloop_clauses + (OmpinReduction,)),
            **kwargs,
        )
...
_valid_single_clauses = (OmpPrivate, OmpFirstPrivate, OmpNowait)

_valid_atomic_clauses = (OmpAtomicClause, AtomicMemoryClause)
...
_valid_target_clauses = (OmpPrivate, OmpLastPrivate, OmpinReduction, OmpDepend, OmpMap)

_valid_teams_clauses = (
    OmpPrivate,
    OmpLastPrivate,
    OmpShared,
    OmpReduction,
...
_valid_Distribute_clauses = (OmpPrivate, OmpFirstPrivate, OmpLastPrivate, OmpCollapse)

_valid_simd_clauses = (OmpLinear, OmpReduction, OmpCollapse, OmpLastPrivate)

_valid_taskloop_clauses = (
    OmpShared,
    OmpPrivate,
    OmpFirstPrivate,
    OmpLastPrivate,
...
if __name__ == "__main__":
    ...
    print(
        parse(
            stmts="#$omp do private ( ipart, pos, spana, lefta, righta, valuesa, spanb, leftb, rightb, valuesb, E)"
        )
    )
    print(
        parse(
            stmts="#$omp do private(ipart, pos, spana, lefta, righta, valuesa, spanb, leftb, rightb, valuesb,E, B)"
        )
    )
