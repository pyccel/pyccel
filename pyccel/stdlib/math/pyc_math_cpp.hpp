 ...
 * LICENSE file  */
/* or go to https://github.com/pyccel/pyccel/blob/devel/LICENSE for full license
 * details. */
/* --------------------------------------------------------------------------------------
 */
#pragma once
...
inline double pyc_radians(double degrees) { return degrees * (M_PI / 180); }
inline double pyc_degrees(double radians) { return radians * (180.0 / M_PI); }
...
Floating pyc_modulo(Floating a, Floating b) {
  assert(b != 0);
  Floating modulo = std::fmod(a, b); // cppcheck-suppress invalidFunctionArg
  if (!((a < 0) ^ (b < 0)) || modulo == 0)
    return modulo;
  else
    return modulo + b;
...
template <class T> std::complex<T> sign(std::complex<T> x) {
...
template <class T> T py_floor_div(T x, T y) {
