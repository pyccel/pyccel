...
def setup_pyccel_config_parser(parser):
    ...
    export_parser = subparsers.add_parser(
        "export", add_help=False, help="Export a compiler configuration to a json file."
    )
    export_parser.add_argument(
        "filename",
        metavar="FILE",
        type=path_with_suffix((".json",), must_exist=False),
    ...
    check_parser = subparsers.add_parser(
        "check", add_help=False, help="Check that a compiler configuration is valid."
    )
    check_parser.add_argument(
        "filename",
        metavar="FILE",
        type=path_with_suffix((".json",)),
    ...
    description = (
        "Register a commonly used compiler configuration. "
    ...
    register_parser = subparsers.add_parser(
        "register",
        add_help=False,
        help="Register a commonly used compiler configuration.",
    ...
    remove_parser = subparsers.add_parser(
        "remove",
        add_help=False,
        help="Remove a registered compiler configuration and the corresponding pre-compiled dependencies.",
    )
...
def check_config_paths(config, descriptor):
    ...
    for inc in config.get("include", ()):
        ...
        if not inc_path.is_absolute():
            print(
                f"|{ERROR}|: include path {inc} for {descriptor} should be absolute",
                file=sys.stderr,
            )
            exitcode = 1
        elif not inc_path.exists():
            print(
                f"|{ERROR}|: include path {inc} for {descriptor} was not found",
                file=sys.stderr,
            )
            exitcode = 1
    for libdir in config.get("libdir", ()):
        ...
        if not libdir_path.is_absolute():
            print(
                f"|{ERROR}|: library directory path {libdir} for {descriptor} should be absolute",
                file=sys.stderr,
            )
            exitcode = 1
        elif not libdir_path.exists():
            print(
                f"|{ERROR}|: library directory path {libdir} for {descriptor} was not found",
                file=sys.stderr,
            )
            exitcode = 1
    for lib in config.get("libs", ()):
        ...
        if len(lib_path.parts) > 1:
            if not lib_path.is_absolute():
                print(
                    f"|{ERROR}|: library {lib} for {descriptor} should be a library name or an absolute path",
                    file=sys.stderr,
                )
                exitcode = 1
            elif not lib_path.exists():
                print(
                    f"|{ERROR}|: library {lib} for {descriptor} should be a library name or should be a path to an existing file",
                    file=sys.stderr,
                )
                exitcode = 1
    for dep in config.get("dependencies", ()):
        ...
        if not dep_path.is_absolute():
            print(
                f"|{ERROR}|: library directory path {dep} for {descriptor} should be absolute",
                file=sys.stderr,
            )
            exitcode = 1
        elif not dep_path.exists():
            print(
                f"|{ERROR}|: library directory path {dep} for {descriptor} was not found",
                file=sys.stderr,
            )
            exitcode = 1
...
def pyccel_config_check(filename):
    ...
    for lang, lang_config in config_contents.items():
        ...
        possible_keys = {
            k for k, v in example_config.items() if not isinstance(v, dict)
        }
        possible_accelerators = {
            k for k, v in example_config.items() if isinstance(v, dict)
        }
        found_keys = set()
        ...
        for k, v in lang_config.items():
            if k in possible_keys:
                ...
                print(
                    f"|{WARNING}|: Key {k} in language {lang} is unrecognised",
                    file=sys.stderr,
                )

        for k in possible_keys.difference(found_keys):
            print(
                f"|{WARNING}|: Key {k} not provided for language {lang}. It will default to an empty value",
                file=sys.stderr,
            )
        ...
        for k in found_keys:
            if not isinstance(lang_config[k], type(example_config[k])):
                print(
                    f"|{ERROR}|: Key {k} in language {lang} is associated with a value of the wrong type.",
                    file=sys.stderr,
                )
        ...
        for acc_name in found_accelerators:
            ...
            possible_keys = (
                accelerator_keys + ("shared_suffix",)
                if acc_name == "python"
                else accelerator_keys
            )
            for k, v in acc.items():
                if k not in possible_keys:
                    print(
                        f"|{WARNING}|: Key {k} for accelerator {acc_name} in language {lang} is unrecognised",
                        file=sys.stderr,
                    )
                else:
                    if k == "shared_suffix":
                        if not isinstance(v, str):
                            print(
                                f"|{ERROR}|: Key {k} for accelerator {acc_name} in language {lang} is associated with a value of the wrong type.",
                                file=sys.stderr,
                            )
                            print("Expected: str", file=sys.stderr)
                            exitcode = 1
                    elif not isinstance(v, list) or not isinstance(
                        next(iter(v), ""), str
                    ):
                        print(
                            f"|{ERROR}|: Key {k} for accelerator {acc_name} in language {lang} is associated with a value of the wrong type.",
                        ...
                        exitcode = 1
                exitcode = exitcode or check_config_paths(
                    acc, f"accelerator {acc_name} in language {lang}"
                )
...
def pyccel_config_register(compiler_family, filename, verbose, conda_warnings):
    ...
    pyccel_config_home = pathlib.Path(
    ...
    if not config_dirpath.is_relative_to(pyccel_config_home):
        print(
            f"|{ERROR}|: The identifier for this compiler family is invalid as it leads to files being created outside the PYCCEL_CONFIG_HOME directory."
        )
        sys.exit(1)
    try:
        ...
        print(
            f"|{ERROR}|: A compiler with the chosen compiler family name is already registered.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    recognised_libs["stc"].install_to(
        config_dirpath, installed_libs, verbose, compiler, use_pkg_config=False
    )
...
def pyccel_config_remove(compiler_family):
    ...
    if not config_dirpath.is_relative_to(pyccel_config_home):
        print(
            f"|{ERROR}|: The identifier for this compiler family is invalid as it refers to files outside the PYCCEL_CONFIG_HOME directory.",
            file=sys.stderr,
        )
        sys.exit(1)
    if config_dirpath.exists():
        ...
        print(
            f"|{ERROR}|: Configuration not found : {compiler_family}", file=sys.stderr
        )
        sys.exit(1)
