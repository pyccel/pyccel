...
STACK_ARRAY_UNKNOWN_SHAPE = (
    "Cannot create stack array from a shape which is unknown at function entry"
)
STACK_ARRAY_SHAPE_UNPURE_FUNC = (
    "Cannot create stack array from a shape created with an impure function"
)
...
TYPEDDICT_KEY_MUST_BE_STRING_LITERAL = "Expected TypedDict key to be string literal"
...
WRONG_NUMBER_OUTPUT_ARGS = (
    "Number of output arguments does not match number of provided variables"
)
...
UNDEFINED_WITH_ACCESS = (
    "The __enter__ or __exit__ method for the with construct cannot be found"
)
...
PYCCEL_RESTRICTION_TRY_EXCEPT_FINALLY = (
    "Uncovered try/except/finally statements by Pyccel"
)
...
PYCCEL_RESTRICTION_IS_ISNOT = (
    "Only booleans and None are allowed as rhs/lhs for is/isnot statement"
)
...
PYCCEL_RESTRICTION_MULTIPLE_COMPARISONS = (
    "Uncovered multi operator comparison statement"
)
...
PYCCEL_RESTRICTION_INHOMOG_LIST = (
    "Inhomogeneous lists are not supported by Pyccel. Please use a tuple"
)
...
ALLOCATABLE_IN_EXPRESSION = "Memory allocation should not be used in an expression."
...
FORTRAN_ELEMENTAL_SINGLE_ARGUMENT = (
    "Elemental functions are defined as scalar operators, with a single dummy argument"
)
...
IMPORTING_EXISTING_IDENTIFIED = "Trying to import an identifier that already exists in the namespace. Hint: use import as"
...
UNDERSCORE_NOT_A_THROWAWAY = (
    "Variables named '_' are assumed to be throwaways so their value cannot be used"
)
...
ARRAY_REALLOCATION = (
    "{class_type} redefinition may cause memory reallocation at runtime"
)
...
NON_LITERAL_KEEP_DIMS = (
    "keep_dims argument must be a literal, otherwise rank is unknown"
)
