    ...
    def install_to(self, pyccel_dirpath, installed_libs, verbose, compiler):
        ...
        with lock:
            ...
            if not lib_dest_path.exists():
                ...
                _, mismatch, _ = filecmp.cmpfiles(self._src_dir, lib_dest_path, src_files)
        ...
        with FileLock(install_dir.with_suffix('.lock')):
            ...
            if not build_dir.exists():
                ...
                if has_meson:
                    ...
                    print("Using compiler : ", Path(compiler.get_exec({}, "c")).name)
                    p = subprocess.run([Path(compiler.get_exec({}, "c")).name, '--version'], capture_output = True, check = False)
                    print(p.stdout)
                    print(p.stderr)
