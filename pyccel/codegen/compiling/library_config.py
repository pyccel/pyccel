...
class ExternalLibInstaller:
    ...
    def _check_for_package(self, pkg_name, options=()):
        ...
        p = subprocess.run(
            [pkg_config, pkg_name, "--cflags-only-I"],
            capture_output=True,
            text=True,
            check=True,
        )
        ...
        p = subprocess.run(
            [pkg_config, pkg_name, "--cflags-only-other"],
            capture_output=True,
            text=True,
            check=True,
        )
        ...
        p = subprocess.run(
            [pkg_config, pkg_name, "--libs-only-other"],
            capture_output=True,
            text=True,
            check=True,
        )
...
class STCInstaller(ExternalLibInstaller):
    ...
    def __init__(self):
        ...
        self._compile_obj = CompileObj(
            "stc",
            folder=self._src_dir.name,
            has_target_file=False,
            include=("include",),
            libdir=("lib/*",),
        )
    ...
    def install_to(
        self, pyccel_dirpath, installed_libs, verbose, compiler, *, use_pkg_config=True
    ):
        """
        Install the files to the Pyccel dirpath.
        ...
        try:
            stc_installation = importlib.resources.files(
                f"pyccel.extensions.stc_install_{compiler_family}"
            )
        ...
        custom_compiler_path = (
            Path(os.environ.get("PYCCEL_CONFIG_HOME", Path.home() / ".pyccel"))
            / compiler_family
            / "STC"
        )
