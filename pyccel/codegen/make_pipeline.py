...
build_system_handler = {"cmake": CMakeHandler, "meson": MesonHandler}
...
def execute_pyccel_make(
    ...
    *,
    ...
    if len(files) == 0:
        errors.report("No files passed to pyccel make", severity="fatal")
    else:
        print("Translating :")
        for f in files:
            print("-", f)
    ...
    for f, p in parsers.items():
        ...
        p.parse(
            verbose=verbose,
            d_parsers_by_filename={f.absolute(): p for f, p in parsers.items()},
        )
    ...
    for f, p in parsers.items():
        ...
        if language == "python":
            ...
            target_name = (
                "__".join(relative_name.parts)
                if has_conflicting_modules
                else relative_name.stem
            )
            targets[f.absolute()] = CompileTarget(
                target_name,
                f.absolute(),
                fname,
                dict(zip(wrapper_files, wrappergen.get_additional_imports())),
                prog_name,
