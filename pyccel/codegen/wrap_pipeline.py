...
def execute_pyccel_wrap(
    ...
    *,
    ...
    if fname.stem in python_builtin_libs:
        raise ValueError(
            f"File called {fname.name} has the same name as a Python built-in package and can't be imported from Python. See #1402"
        )
    ...
    includes = [
        os.path.join(pymod_dirpath, i.strip())
        for i in parser.metavars.get("includes", "").split(",")
        if i
    ]
    libdirs = [
        os.path.join(pymod_dirpath, l.strip())
    ...
    for i, (obj, lang, imports) in enumerate(
        zip(
            wrapper_compile_objs, printed_languages, wrappergen.get_additional_imports()
        )
    ):
        obj.add_dependencies(*wrapper_compile_objs[:i])
        manage_dependencies(imports, compiler, pyccel_dirpath, obj, lang, verbose)
