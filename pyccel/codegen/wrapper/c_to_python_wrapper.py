...
class CToPythonWrapper(Wrapper):
    ...
    def _get_python_argument_variables(self, args):
        ...
        is_bound = [
            getattr(a, "wrapping_bound_argument", a.bound_argument) for a in args
        ]
        collect_args = [
    ...
    def _unpack_python_args(self, args, class_base=None):
        ...
        func_args = [self.get_new_PyObject("self", class_base)] + [
            self.get_new_PyObject(n) for n in ("args", "kwargs")
        ]
        ...
        arg_names = [
            "" if a.is_posonly else getattr(a.var, "original_var", a.var).name
            for a in args
        ]
        ...
        body = [
            ...
            for func_def_arg, py_arg in zip(args, arg_vars)
            if func_def_arg.has_default
        ]

        body.append(keyword_list)
        body.append(
            If(IfSection(PyccelNot(parse_node), [Return(self._error_exit_code)]))
        )
    ...
    def _get_type_check_condition(
        self, py_obj, arg, raise_error, body, allow_empty_arrays
    ):
        """
        ...
        if isinstance(dtype, CustomDataType):
            ...
            func = FunctionDef(
                ...
                arguments=[
                    FunctionDefArgument(
                        Variable(PyccelPyObject(), name="o", memory_handling="alias")
                    )
                ],
            ...
            indexed_py_obj = self.scope.get_temporary_variable(
                PyccelPyObject(), memory_handling="alias"
            )
            iter_obj = self.scope.get_temporary_variable(
                PyccelPyObject(), "iter", memory_handling="alias"
            )
            ...
            for_body.append(
                Assign(
                    ...
                    PyccelAnd(type_check_condition, internal_type_check_condition),
                )
            )
            ...
            if orig_funcs[0].arguments[i].has_default:
                ...
                for func, a in zip(funcs, interface_args):
                    index = (
                        next(
                            i
                            for i, p_t in enumerate(possible_types)
                            if p_t is a.class_type
                        )
                        * step
                    )
        ...
        if self._error_exit_code is Nil():
            ...
            func_results = FunctionDefResult(
                self.scope.get_temporary_variable(
                    self._error_exit_code.class_type, "result"
                )
            )
    ...
    def _save_referenced_objects(self, func, func_args):
        ...
        for a in func.arguments:
            if a.persistent_target:
                ref_attribute = class_scope.find(
                    "referenced_objects", "variables", raise_if_missing=True
                )
                ref_list = ref_attribute.clone(
                    ref_attribute.name, new_class=DottedVariable, lhs=class_arg_var
                )
                python_arg = self._python_object_map[a]
                if not isinstance(python_arg.dtype, PyccelPyObject):
                    python_arg = ObjectAddress(
                        PointerCast(python_arg, PyList_Append.arguments[1].var)
                    )
                ...
                body.extend(
                    [
                        If(
                            IfSection(
                                PyccelEq(
                                    append_call, PyccelUnarySub(LiteralInteger(1))
                                ),
                                [Return(self._error_exit_code)],
                            )
                        )
    ...
    def _incref_return_pointer(self, ref_obj, return_var, orig_var):
        ...
        if isinstance(orig_var.class_type, NumpyNDArrayType):
            save_ref_call = PyArray_SetBaseObject(
                ObjectAddress(
                    PointerCast(return_var, PyArray_SetBaseObject.arguments[0].var)
                ),
                ObjectAddress(
                    PointerCast(ref_obj, PyArray_SetBaseObject.arguments[1].var)
                ),
            )
            return [
                Py_INCREF(ref_obj),
                If(
                    IfSection(
                        PyccelLt(save_ref_call, LiteralInteger(0, dtype=CNativeInt())),
                        [Return(self._error_exit_code)],
                    )
                ),
            ]
        elif isinstance(orig_var.dtype, CustomDataType):
            ref_attribute = return_var.cls_base.scope.find(
                "referenced_objects", "variables", raise_if_missing=True
            )
            ref_list = ref_attribute.clone(
                ref_attribute.name, new_class=DottedVariable, lhs=return_var
            )
            save_ref_call = PyList_Append(
                ref_list, ObjectAddress(PointerCast(ref_obj, ref_list))
            )
            return [
                If(
                    IfSection(
    ...
    def _add_object_to_mod(self, module_var, obj, name, initialised):
        ...
        if_expr = If(
            IfSection(
                PyccelLt(add_expr, LiteralInteger(0)),
    ...
    def _build_module_init_function(self, expr, imports, module_def_name):
        ...
        """
        mod_name = self.scope.get_python_name(
            getattr(expr, "original_module", expr).name
        )
        ...
        API_var_name = self.scope.get_new_name(
        ...
        body.extend(
            self._add_object_to_mod(module_var, capsule_obj, "_C_API", initialised)
        )

        body.append(import_array())
        import_funcs = [
            i.source_module.import_func
            for i in imports
            if isinstance(i.source_module, PyModule)
        ]
        for i_func in import_funcs:
            body.append(
                If(
                    IfSection(
                        PyccelLt(i_func(), ok_code),
        ...
        for i, c in enumerate(expr.classes):
            ...
            if_expr = If(
                IfSection(
                    PyccelLt(ready_type, LiteralInteger(0)),
                    [Py_DECREF(i) for i in initialised]
                    + [Return(self._error_exit_code)],
                )
            )
            ...
            body.extend(
                self._add_object_to_mod(
                    module_var, type_object, class_name, initialised
                )
            )
        ...
        for v in expr.variables:
            ...
            body.extend(
                self._add_object_to_mod(module_var, wrapped_var, var_name, initialised)
            )
    ...
    def _build_module_import_function(self, expr):
        ...
        current_path = func_scope.get_temporary_variable(
            PyccelPyObject(), "current_path", memory_handling="alias"
        )
        stash_path = func_scope.get_temporary_variable(
            PyccelPyObject(), "stash_path", memory_handling="alias"
        )
    ...
    def _allocate_class_instance(self, class_var, scope, is_alias):
        ...
        ref_attribute = scope.find(
            "referenced_objects", "variables", raise_if_missing=True
        )
        ref_list = ref_attribute.clone(
            ref_attribute.name, new_class=DottedVariable, lhs=class_var
        )

        # Get alias attribute
        attribute = scope.find("is_alias", "variables", raise_if_missing=True)
        alias_bool = attribute.clone(
    ...
    def _get_class_allocator(self, class_dtype, func=None):
        ...
        if func:
            func_name = self.scope.get_new_name(
                f"{func.name}__wrapper", object_type="wrapper"
            )
        else:
        ...
        c_res = attribute.clone(
            attribute.name, new_class=DottedVariable, lhs=python_result_var
        )
    ...
    def _get_class_initialiser(self, init_function, cls_dtype):
        ...
        # Get the results of the PyFunctionDef
        python_result_variable = Variable(
            CNativeInt(), self.scope.get_new_name(), is_temp=True
        )
    ...
    def _get_class_destructor(self, del_function, cls_dtype, wrapper_scope):
        ...
        is_alias = attribute.clone(
            attribute.name, new_class=DottedVariable, lhs=func_arg
        )
        ...
        # Get the list of referenced objects
        ref_attribute = wrapper_scope.find(
            "referenced_objects", "variables", raise_if_missing=True
        )
        ref_list = ref_attribute.clone(
            ref_attribute.name, new_class=DottedVariable, lhs=func_arg
        )
    ...
    def _get_array_parts(self, orig_var, collect_arg):
        ...
        """
        pyarray_collect_arg = PointerCast(
            collect_arg, Variable(PyccelPyArrayObject(), "_", memory_handling="alias")
        )
        data_var = Variable(
            VoidType(),
        ...
        get_data = AliasAssign(
            data_var, PyArray_DATA(ObjectAddress(pyarray_collect_arg))
        )
        ...
        if n_targets == 1:
            ...
            if isinstance(orig_var.class_type, NumpyNDArrayType):
                ...
                for t in arg_targets:
                    ...
                    body.extend(
                        self._incref_return_pointer(collect_arg, python_res, orig_var)
                    )
                return body
        return []
    ...
    def _wrap_Module(self, expr):
        ...
        imports = [
            self._wrap(i) for i in getattr(expr, "original_module", expr).imports
        ]
    ...
    def _wrap_BindCModule(self, expr):
        ...
        for v in expr.variable_wrappers:
            ...
            external_funcs.append(
                FunctionDef(
                    f.name, f.arguments, [], f.results, is_header=True, scope=f.scope
                )
            )
        ...
        for c in expr.classes:
            ...
            external_funcs.append(
                FunctionDef(
                    m.name, m.arguments, [], m.results, is_header=True, scope=m.scope
                )
            )
            for m in c.methods:
                external_funcs.append(
    ...
    def _wrap_Interface(self, expr):
        ...
        type_check_name = self.scope.get_new_name(
            expr.name + "_type_check", object_type="wrapper"
        )
        ...
        for func, index in argument_type_flags.items():
            ...
            if_sections.append(
                IfSection(
                    PyccelEq(type_indicator, LiteralInteger(index)),
        ...
        if_sections.append(
            IfSection(
                PyccelEq(type_indicator, PyccelUnarySub(LiteralInteger(1))),
                [Return(self._error_exit_code)],
            )
        )
        if_sections.append(
    ...
    def _wrap_FunctionDef(self, expr):
        ...
        in_interface = (
            len(expr.get_user_nodes(Interface, excluded_nodes=(FunctionCall,))) > 0
        )
        ...
        if "property" in original_func.decorators:
            ...
            if (
                in_interface
                or original_func_name in magic_binary_funcs
                or original_func_name == "__len__"
            ):
                func_args = [
                    FunctionDefArgument(a)
                    for a in self._get_python_argument_variables(python_args)
                ]
                body = []
            else:
                func_args, body = self._unpack_python_args(python_args, class_dtype)
        ...
        if original_func_name in magic_binary_funcs and original_func_name.startswith(
            "__i"
        ):
            res = func_args[0].var.clone(
                self.scope.get_new_name(func_args[0].var.name), is_argument=False
            )
            wrapped_results = {"c_results": [], "py_result": res, "body": []}
            body.append(AliasAssign(res, func_args[0].var))
            body.append(Py_INCREF(res))
        else:
            wrapped_results = self._extract_FunctionDefResult(
                python_results.var, is_bind_c_function_def, expr
            )
    ...
    def _wrap_FunctionDefArgument(self, expr):
        ...
        arg_extraction = self._extract_FunctionDefArgument(
            orig_var, collect_arg, bound_argument, is_bind_c_argument
        )
        ...
        if expr.has_default:
            ...
            body.append(
                If(
                    IfSection(
                        PyccelIsNot(collect_arg, Py_None),
                        [
                            If(
            ...
            body.append(
                If(
                    IfSection(
                        PyccelNot(check_func), [*err, Return(self._error_exit_code)]
                    )
                )
            )
            body.extend(cast)
    ...
    def _wrap_Variable(self, expr):
        ...
        py_equiv = self.scope.get_temporary_variable(
            PyccelPyObject(), memory_handling="alias"
        )
    ...
    def _wrap_DottedVariable(self, expr):
        ...
        class_ptr_attrib = class_scope.find(
            "instance", "variables", raise_if_missing=True
        )
        ...
        # Cast the C variable into a Python variable
        result_wrapping = self._extract_FunctionDefResult(
            expr.clone(expr.name, new_class=Variable), False
        )
        ...
        setter_result = FunctionDefResult(
            setter_scope.get_temporary_variable(CNativeInt())
        )
        self.scope.insert_symbol(expr.name)
    ...
    def _wrap_BindCClassProperty(self, expr):
        ...
        # ----------------------------------------------------------------------------------
        getter_name = self.scope.get_new_name(
            f"{class_type.name}_{name}_getter", object_type="wrapper"
        )
        getter_scope = self.scope.new_child_scope(getter_name, "function")
        self.scope = getter_scope
        ...
        # Cast the C variable into a Python variable
        get_val_result_var = getattr(
            get_val_result, "original_function_result_variable", get_val_result.var
        )
        result_wrapping = self._extract_FunctionDefResult(
            get_val_result_var, True, expr.getter
        )
        res_wrapper = result_wrapping["body"]
        ...
        if isinstance(expr.getter.original_function, DottedVariable):
            ...
            res_wrapper.extend(
                self._incref_return_pointer(getter_args[0], getter_result, wrapped_var)
            )
        else:
        ...
        # ----------------------------------------------------------------------------------
        if expr.setter:
            self._error_exit_code = PyccelUnarySub(
                LiteralInteger(1, dtype=CNativeInt())
            )
            setter_name = self.scope.get_new_name(
            ...
            setter_args = [
                ...
                setter_scope.get_temporary_variable(
                    VoidType(), memory_handling="alias"
                ),
            ]
            setter_result = FunctionDefResult(
                setter_scope.get_temporary_variable(CNativeInt())
            ...
            if (
                isinstance(wrapped_var.class_type, FixedSizeNumericType)
                or wrapped_var.is_alias
            ):
    ...
    def _wrap_ClassDef(self, expr):
        ...
        for f in expr.methods:
            ...
            if python_name == "__del__":
                wrapped_class.add_new_method(
                    self._get_class_destructor(f, orig_cls_dtype, wrapped_class.scope)
                )
            elif python_name == "__init__":
                wrapped_class.add_new_method(
                    self._get_class_initialiser(f, orig_cls_dtype)
                )
        ...
        if bound_class:
            wrapped_class.add_alloc_method(
                self._get_class_allocator(orig_cls_dtype, expr.new_func)
            )
        else:
        ...
        for a in expr.attributes:
            ...
            if bound_class or not a.is_private:
                if isinstance(a, (DottedVariable, BindCClassProperty)):
                    ...
                    wrapped_class.add_property(
                        self._wrap(
                            a.clone(a.name, new_class=DottedVariable, lhs=pseudo_self)
                        )
                    )
    ...
    def _extract_FunctionDefArgument(
    ...
    def _extract_FixedSizeType_FunctionDefArgument(
        ...
        if getattr(orig_var, "is_optional", False):
            memory_var = self.scope.get_temporary_variable(
                arg_var, name=arg_var.name + "_memory", is_optional=False
            )
    ...
    def _extract_NumpyNDArrayType_FunctionDefArgument(
        ...
        body.append(
            Allocate(
                arg_var, shape=tuple(shape_elems), status="unallocated", like=args[0]
            )
        )
        body.append(
            AliasAssign(
                ...
                IndexedElement(
                    arg_var,
                    *[Slice(None, u, s) for s, u in zip(stride_elems, ubound_elems)],
                ),
            )
        )
        ...
        if orig_var.is_optional:
            optional_arg_var = sliced_arg_var.clone(
                self.scope.get_expected_name(orig_var.name), is_optional=True
            )
    ...
    def _extract_HomogeneousTupleType_FunctionDefArgument(
        self, orig_var, collect_arg, bound_argument, is_bind_c_argument, *, arg_var=None
    ):
        """
        ...
        if orig_var.rank > 1:
            errors.report(
                "Wrapping multi-level tuples is not yet supported",
                severity="fatal",
                symbol=orig_var,
            )
        ...
        if orig_var.is_optional:
            errors.report(
                "Optional tuples are not yet supported",
                severity="fatal",
                symbol=orig_var,
            )

        size_var = self.scope.get_temporary_variable(
            PythonNativeInt(), self.scope.get_new_name(f"{orig_var.name}_size")
        )

        if is_bind_c_argument:
            data_var = Variable(
                CStackArray.get_new(orig_var.class_type.element_type),
                self.scope.get_new_name(orig_var.name + "_data"),
                memory_handling="alias",
            )
            self.scope.insert_variable(data_var)
            ...
            self.scope.insert_symbolic_alias(
                IndexedElement(arg_var, LiteralInteger(0)), ObjectAddress(data_var)
            )
            self.scope.insert_symbolic_alias(
                IndexedElement(arg_var, LiteralInteger(1)), size_var
            )
            fill_var = data_var
        ...
        indexed_collect_arg = self.scope.get_temporary_variable(
            PyccelPyObject(), memory_handling="alias"
        )

        body = [
            Assign(size_var, PyTuple_Size(collect_arg)),
        ...
        for_body += self._extract_FunctionDefArgument(
            indexed_orig_var,
            indexed_collect_arg,
            bound_argument,
            is_bind_c_argument,
            arg_var=indexed_arg_var,
        )["body"]
        self.exit_scope()
    ...
    def _extract_HomogeneousSetType_FunctionDefArgument(
        self, orig_var, collect_arg, bound_argument, is_bind_c_argument, *, arg_var=None
    ):
        assert arg_var is None
        ...
        if orig_var.is_optional:
            errors.report(
                "Optionals are not yet supported", severity="fatal", symbol=orig_var
            )
        ...
        if is_bind_c_argument:
            ...
            self.scope.insert_symbolic_alias(
                IndexedElement(arg_var, LiteralInteger(0)), data
            )
            self.scope.insert_symbolic_alias(
                IndexedElement(arg_var, LiteralInteger(1)), size_var
            )
        ...
        idx = self.scope.get_temporary_variable(CNativeInt())
        indexed_orig_var = self.scope.get_temporary_variable(
            orig_var.class_type.element_type
        )
        indexed_collect_arg = self.scope.get_temporary_variable(
            PyccelPyObject(), memory_handling="alias"
        )
        ...
        if not isinstance(orig_var.class_type, FinalType):
            if is_bind_c_argument:
                errors.report(
                    "Python built-in containers should be passed as constant arguments when "
                ...
                element_extraction = self._extract_FunctionDefResult(
                    IndexedElement(orig_var, idx), is_bind_c_argument, None
                )
                ...
                for_body = [
                    *element_extraction["body"],
                    If(
                        IfSection(
                            PyccelEq(elem_set, PyccelUnarySub(LiteralInteger(1))),
                            [Return(self._error_exit_code)],
                        )
                    ),
                ]
                ...
                clean_up = [
                    ...
                    For(
                        (element_extraction["c_results"][0],),
                        loop_iterator,
                        for_body,
                        for_scope,
                    ),
    ...
    def _extract_HomogeneousListType_FunctionDefArgument(
        self, orig_var, collect_arg, bound_argument, is_bind_c_argument, *, arg_var=None
    ):
        assert arg_var is None
        ...
        if not isinstance(orig_var.class_type, FinalType):
            if is_bind_c_argument:
                errors.report(
                    "Lists should be passed as constant arguments when translating to languages other than C. "
    ...
    def _extract_CustomDataType_FunctionDefResult(
        self, wrapped_var, is_bind_c, funcdef
    ):
        """
        ...
        setup = self._allocate_class_instance(
            python_res, python_res.cls_base.scope, orig_var.is_alias
        )
        if is_bind_c:
            ...
            attribute = scope.find("instance", "variables", raise_if_missing=True)
            attrib_var = attribute.clone(
                attribute.name, new_class=DottedVariable, lhs=python_res
            )
            ...
            setup.append(
                Allocate(c_res, shape=None, status="unallocated", like=orig_var)
            )
        ...
        if funcdef:
            body.extend(
                self.connect_pointer_targets(orig_var, python_res, funcdef, is_bind_c)
            )
    ...
    def _extract_NumpyNDArrayType_FunctionDefResult(self, orig_var, is_bind_c, funcdef):
        ...
        if funcdef:
            arg_targets = funcdef.result_pointer_map.get(orig_var, ())
            release_memory = len(arg_targets) == 0 and not isinstance(
                orig_var, DottedVariable
            )
        body = [
            AliasAssign(
                py_res,
    ...
    def _extract_InhomogeneousTupleType_FunctionDefResult(
        ...
        extract_elems = [
            self._extract_FunctionDefResult(
                funcdef.scope.collect_tuple_element(e), is_bind_c, funcdef
            )
            for e in c_compatible_var
        ]
        ...
        body.append(
            AliasAssign(
                py_res, PyTuple_Pack(*[ObjectAddress(r) for r in py_result_vars])
            )
        )
        body.extend(Py_DECREF(r) for r in py_result_vars)

        return {
            "c_results": PythonTuple(*c_result_vars),
            "py_result": py_res,
            "body": body,
            "setup": setup,
        }

    def _extract_HomogeneousContainerType_FunctionDefResult(
        self, wrapped_var, is_bind_c, funcdef
    ):
        """
        ...
        if isinstance(wrapped_var, PythonTuple):
            return self._extract_InhomogeneousTupleType_FunctionDefResult(
                wrapped_var, is_bind_c, funcdef
            )
        ...
        if is_bind_c:
            ...
            loop_size = shape_var.clone(
                self.scope.get_new_name(shape_var.name), is_argument=False
            )
            ...
            c_results = [c_res]
            loop_size = Variable(
                PythonNativeInt(), self.scope.get_new_name(f"{name}_size")
            )
        ...
        self.scope = for_scope
        element_extraction = self._extract_FunctionDefResult(
            IndexedElement(orig_var, idx), is_bind_c, funcdef
        )
        self.exit_scope()
        ...
        if isinstance(class_type, HomogeneousSetType):
            ...
            raise NotImplementedError(
                f"Don't know how to return an object of type {class_type}"
            )

        for_body = [
            Assign(element_extraction["c_results"][0], element),
            *element_extraction["body"],
            If(
                IfSection(
                    PyccelEq(elem_set, PyccelUnarySub(LiteralInteger(1))),
    ...
    def _extract_DictType_FunctionDefResult(self, wrapped_var, is_bind_c, funcdef):
        ...
        if is_bind_c:
            ...
            key_c_res = Variable(
                CStackArray.get_new(orig_var.class_type.key_type),
                self.scope.get_new_name(key_ptr_var.name),
            )
            val_c_res = Variable(
                CStackArray.get_new(orig_var.class_type.value_type),
            ...
            self.scope.insert_variable(c_res)
            iterable.set_loop_counter(
                Variable(PythonNativeInt(), self.scope.get_new_name())
            )
        ...
        elem_set = PyDict_SetItem(
            py_res, key_extraction["py_result"], value_extraction["py_result"]
        )
        for_body = [
            *key_extraction["body"],
            *value_extraction["body"],
            If(
                IfSection(
                    PyccelEq(elem_set, PyccelUnarySub(LiteralInteger(1))),
                    [Return(self._error_exit_code)],
        ...
        if is_bind_c:
            for_body = [
                Assign(key_extraction["c_results"][0], key_elem),
                Assign(value_extraction["c_results"][0], val_elem),
            ] + for_body
        self.exit_scope()
        for_loop = For(
            (key_extraction["c_results"][0], value_extraction["c_results"][0]),
            iterable,
            for_body,
            for_scope,
        )

        body = [AliasAssign(py_res, PyDict_New()), for_loop]
    ...
    def _extract_StringType_FunctionDefResult(self, wrapped_var, is_bind_c, funcdef):
        ...
        if is_bind_c:
            ...
            c_res = Variable(
                StringType(), self.scope.get_new_name(name), memory_handling="heap"
            )
            self.scope.insert_variable(c_res)
