...
class CCodePrinter(CodePrinter):
    ...
    def is_c_pointer(self, a):
        ...
        if (
            ...
            and len(a.indices) == a.base.class_type.container_rank
        ):
            return True
        ...
        if (
            isinstance(
                a.class_type, (CustomDataType, HomogeneousContainerType, DictType)
            )
            and a.is_argument
    ...
    def copy_NumpyArray_Data(self, lhs, rhs):
        ...
        variables = [
            v
            for v in arg.get_attribute_nodes((Variable, FunctionCall, PyccelFunction))
            if v.rank
        ]
        ...
        if isinstance(arg, (Variable, IndexedElement)):
            ...
            if isinstance(arg, IndexedElement):
                arg_var = self.scope.get_temporary_variable(
                    arg.class_type, memory_handling="heap"
                )
            ...
            if isinstance(arg.class_type, (NumpyNDArrayType, HomogeneousTupleType)):
                ...
                return (
                    prefix
                    + f"cspan_copy({cast_type}, {lhs_c_type}, {rhs_c_type}, {lhs_address}, {rhs_address});\n"
                )
            else:
                raise NotImplementedError(
                    f"Can't copy variable of type {arg.class_type}"
                )
        elif variables:
            body = ""
            for li, ri in zip(lhs, arg):
                if li.rank:
                    li_slice_var = self.scope.get_temporary_variable(
                        li.class_type,
                        shape=get_shape_of_multi_level_container(ri),
        ...
        flattened_lhs = [
            get_indexed(lhs, elems)
            for elems in product(
                *[range(s) for s in get_shape_of_multi_level_container(arg)]
            )
        ]
    ...
    def _init_stack_array(self, expr):
        ...
        tot_shape = self._print(
            functools.reduce(PyccelMul.make_simplified, var.alloc_shape)
        )
        ...
        if isinstance(class_type, StringType):
            ...
            for e in elements:
                if isinstance(e, Variable):
                    ...
                    stc_init_elements.append(
                        f"{element_type}_clone({self._print(mem_var)})"
                    )
                else:
    ...
    def _handle_numpy_functional(self, expr, ElementExpression, start_val=None):
        ...
        if assign_node:
            ...
            if not isinstance(arg_var, Variable):
                ...
                assert arg_var.rank
                tmp = self.scope.get_temporary_variable(
                    arg_var.class_type, shape=arg_var.shape, memory_handling="alias"
                )
            ...
            iter_ref_var = DottedVariable(
                arg_var.class_type.element_type,
                "ref",
                memory_handling="alias",
                lhs=iter_var,
            )
            ...
            self._additional_code += self._handle_numpy_functional(
                expr, ElementExpression, start_val
            )
    ...
    def _get_stc_element_type_decl(self, element_type, expr, tag="key", in_arc=False):
        ...
        if in_arc:
            ...
            element_type, (HomogeneousListType, HomogeneousSetType, DictType)
        ):
            type_decl = self.get_c_type(element_type, not in_arc)
            ...
            decl_line = ""
            errors.report(
                f"The declaration of type {element_type} is not yet implemented for containers.",
                symbol=expr,
                severity="error",
            )
        return decl_line
    ...
    def _inline_vecdot(self, expr, x1, x2, axis, *, with_conjugate):
        ...
        for a in in_arg_vars:
            if not isinstance(a, Variable):
                ...
                assert a.rank
                tmp = self.scope.get_temporary_variable(
                    a.class_type, shape=a.shape, memory_handling="alias"
                )
        ...
        if with_conjugate and x1.dtype.primitive_type is PrimitiveComplexType():
            node = self._print(
                AugAssign(
                    lhs_var, "+", PyccelMul(PythonConjugate(x1[iter_var]), x2[iter_var])
                )
            )
        else:
            node = self._print(
                AugAssign(lhs_var, "+", PyccelMul(x1[iter_var], x2[iter_var]))
            )
    ...
    def _print_PythonMinMax(self, expr):
        ...
        variadic_args = isinstance(
            primitive_type, (PrimitiveFloatingPointType, PrimitiveIntegerType)
        )
        can_compare = primitive_type is not PrimitiveComplexType()

        if isinstance(arg, Variable) and isinstance(
            arg.class_type, HomogeneousTupleType
        ):
            if isinstance(arg.shape[0], LiteralInteger):
                arg = PythonTuple(*arg)
            else:
                return errors.report(
                    f"{expr.name} in C does not support tuples of unknown length\n"
                    + PYCCEL_RESTRICTION_TODO,
                    symbol=expr,
                    severity="fatal",
                )
        if isinstance(arg, (PythonTuple, PythonList)) and variadic_args:
            ...
            if (
                ...
                self.add_import(
                    Import(
                        import_loc, AsName(VariableTypeAnnotation(class_type), c_type)
                    )
                )
                return f"{c_type}_{expr.name}({arg_obj})"
            else:
                return errors.report(
                    f"{expr.name} in C does not support arguments of type {arg.class_type}",
                    symbol=expr,
                    severity="fatal",
                )
        if len(arg) != 2:
            return errors.report(
                f"{expr.name} in C does not support {len(arg)} arguments of type {arg.dtype}\n"
                + PYCCEL_RESTRICTION_TODO,
                symbol=expr,
                severity="fatal",
            )
    ...
    def _print_SysExit(self, expr):
        ...
        if (
            not isinstance(
                getattr(expr.status.dtype, "primitive_type", None), PrimitiveIntegerType
            )
    ...
    def _print_LiteralComplex(self, expr):
        if expr.real == LiteralFloat(0):
            ...
            return self._print(
                PyccelAssociativeParenthesis(
                    PyccelAdd(expr.real, PyccelMul(expr.imag, LiteralImaginaryUnit()))
                )
            )
    ...
    def _print_PyccelAnd(self, expr):
        args = [
            (
                ...
                and not isinstance(a, PyccelAssociativeParenthesis)
                else self._print(a)
            )
            for a in expr.args
        ]
    ...
    def _print_PyccelNot(self, expr):
        arg = expr.args[0]
        a = self._print(arg)
        if isinstance(arg, PyccelOperator) and not isinstance(
            arg, PyccelAssociativeParenthesis
        ):
            a = f"({a})"
        return f"!{a}"
    ...
    def _print_Import(self, expr):
        ...
        if source == "stc/cspan":
            ...
            code = ""
            self.add_import(
                Import(stc_extension_mapping[source], (), ignore_at_print=True)
            )
            for t in expr.target:
            ...
            for t in expr.target:
                ...
                if isinstance(class_type, DictType):
                    key_decl_line = self._get_stc_element_type_decl(
                        class_type.key_type, expr
                    )
                    val_decl_line = self._get_stc_element_type_decl(
                        class_type.value_type, expr, "val"
                    )
                    decl_line += key_decl_line + val_decl_line
                elif isinstance(class_type, (HomogeneousListType, HomogeneousSetType)):
                    key_decl_line = self._get_stc_element_type_decl(
                        class_type.element_type, expr
                    )
                    decl_line += key_decl_line
                elif isinstance(class_type, MemoryHandlerType):
                    element_type = self.get_c_type(class_type.element_type)
                    m_decl_line = self._get_stc_element_type_decl(
                        class_type.element_type, expr, in_arc=True
                    )
                    decl_line += m_decl_line
                else:
                    raise NotImplementedError(
                        f"Import not implemented for {container_type}"
                    )
                if (
                    isinstance(class_type, (HomogeneousListType, HomogeneousSetType))
                    and isinstance(class_type.element_type, FixedSizeNumericType)
                    and not isinstance(
                        class_type.element_type.primitive_type, PrimitiveComplexType
                    )
                ):
                ...
                if source in stc_extension_mapping:
                    code += (
                        decl_line + f"#include <{stc_extension_mapping[source]}.h>\n"
                    )
        ...
        for i, f in enumerate(orig_args):
            ...
            if isinstance(f, (PythonTuple, PythonList, PythonSet)):
                start_tag, end_tag = (
                    ...
                    else list_tags if isinstance(f, PythonList) else set_tags
                )
                if args_format:
                ...
                args = [
                    ...
                    for elem in f
                    for print_arg in (elem, container_sep)
                ][:-1]
                if len(f) == 1:
                ...
                code += self._print(
                    PythonPrint([start_tag, *args, end_tag, empty_sep, end_of_block])
                )
                args = []
                continue
            ...
            if isinstance(f, FunctionCall) and isinstance(f.class_type, TupleType):
                tmp_list = [
                    self.scope.get_temporary_variable(a.var.dtype)
                ...
                for_end = FunctionCallArgument(
                    LiteralString("]" + end if i == len(orig_args) - 1 else "]"),
                    keyword="end",
                )
    ...
    def get_c_type(self, dtype, in_container=False):
        ...
        if isinstance(dtype, FixedSizeNumericType):
            ...
            element_type = self.get_c_type(dtype.datatype, in_container=True).replace(
                " ", "_"
            )
            i_type = f"array_{element_type}_{dtype.rank}d"
            self.add_import(
                Import("stc/cspan", AsName(VariableTypeAnnotation(dtype), i_type))
            )
            return i_type

        elif isinstance(dtype, (HomogeneousSetType, HomogeneousListType)):
            container_type = "hset" if dtype.name == "set" else "vec"
            element_type = self.get_c_type(
                dtype.element_type, in_container=True
            ).replace(" ", "_")
            i_type = f"{container_type}_{element_type}"
            self.add_import(
                Import(
                    f"stc/{container_type}",
            ...
            container_type = "hmap"
            key_type = self.get_c_type(dtype.key_type, in_container=True).replace(
                " ", "_"
            )
            val_type = self.get_c_type(dtype.value_type, in_container=True).replace(
                " ", "_"
            )
        ...
        try:
            ...
            raise errors.report(
                PYCCEL_RESTRICTION_TODO,  # pylint: disable=raise-missing-from
                symbol=dtype,
                severity="fatal",
            )
    ...
    def _print_Declare(self, expr):
        ...
        if isinstance(var.class_type, InhomogeneousTupleType):
            return ""
        if (
            get_managed_memory_object(var) != var
            and not var.on_stack
            ...
            if (
                ...
                if managed_mem_lst:
                    ...
                    if managed_var.on_stack:
                        mem_type = self.get_c_type(
                            var.class_type.element_type, in_container=True
                        )
    ...
    def function_signature(self, expr, print_arg_names=True):
        ...
        if n_results > 1 or isinstance(
            expr.results.var.class_type, InhomogeneousTupleType
        ):
    ...
    def _print_IndexedElement(self, expr):
        ...
        if isinstance(base, IndexedElement):
            while isinstance(base, IndexedElement) and isinstance(
                base.class_type, (NumpyNDArrayType, HomogeneousTupleType)
            ):
                inds = list(base.indices) + inds
                base = base.base

        if expr.rank > 0 and isinstance(
            base.class_type, (NumpyNDArrayType, HomogeneousTupleType)
        ):
            ...
            for i, idx in enumerate(inds):
                if isinstance(idx, Slice):
                    idx = self._new_slice_with_processed_arguments(
                        idx, PyccelArrayShapeElement(base, i), allow_negative_indexes
                    )
    ...
    def _print_DottedVariable(self, expr):
        ...
        if isinstance(class_type, CustomDataType):
            ...
            if class_obj:
                ...
                while not class_obj_containing_attrib.scope.find(
                    python_name, "variables", local_only=True
                ):
                    (class_obj_containing_attrib,) = (
                        class_obj_containing_attrib.superclasses
                    )
        else:
            class_obj = None
    ...
    def _new_slice_with_processed_arguments(_slice, array_size, allow_negative_index):
        ...
        if isinstance(explicit_start, PyccelUnarySub) and isinstance(
            explicit_start.args[0], LiteralInteger
        ):
            start = PyccelMinus.make_simplified(array_size, explicit_start.args[0])
        elif allow_negative_index and not isinstance(
            explicit_start, (LiteralInteger, PyccelArrayShapeElement)
        ):
            start = IfTernaryOperator(
                PyccelLt(explicit_start, LiteralInteger(0)),
                PyccelAdd.make_simplified(array_size, explicit_start),
                explicit_start,
            )
        else:
            start = _slice.start

        if isinstance(explicit_stop, PyccelUnarySub) and isinstance(
            explicit_stop.args[0], LiteralInteger
        ):
            stop = PyccelMinus.make_simplified(array_size, explicit_stop.args[0])
        elif allow_negative_index and not isinstance(
            explicit_stop, (LiteralInteger, PyccelArrayShapeElement)
        ):
            stop = IfTernaryOperator(
                PyccelLt(explicit_stop, LiteralInteger(0)),
                PyccelAdd.make_simplified(array_size, explicit_stop),
                explicit_stop,
            )
        else:
            stop = _slice.stop
        ...
        if isinstance(explicit_step, PyccelUnarySub) and isinstance(
            explicit_step.args[0], LiteralInteger
        ):
            start = (
        ...
        # variable step in slice
            ...
            start = IfTernaryOperator(
                PyccelGt(explicit_step, LiteralInteger(0)),
                explicit_start,
    ...
    def _print_Allocate(self, expr):
        ...
        if isinstance(
            ...
            tot_shape = self._print(
                functools.reduce(PyccelMul.make_simplified, expr.shape)
            )
        ...
        for arg in expr.args:
            if arg.dtype.primitive_type is PrimitiveComplexType():
                ...
                try:
                    ...
                    errors.report(
                        INCOMPATIBLE_TYPEVAR_TO_FUNC.format(type_name), severity="fatal"
                    )
    ...
    def _print_NumpySum(self, expr):
        initial = (
            convert_to_literal(0, expr.dtype)
            if expr.initial is None
    ...
    def _print_NumpyNorm(self, expr):
        ...
        code = self._handle_numpy_functional(
            expr,
            lambda tot, elem: reduction_expression(tot, element_expression(elem)),
            initial,
        )
    ...
    def _print_NumpyCross(self, expr):
        ...
        return (
            f"{c_0} = {a_1} * {b_2} - {a_2} * {b_1};\n"
            f"{c_1} = {a_2} * {b_0} - {a_0} * {b_2};\n"
            f"{c_2} = {a_0} * {b_1} - {a_1} * {b_0};\n"
        )

    def _print_NumpyVecdot(self, expr):
        return self._inline_vecdot(
            expr, expr.x1, expr.x2, expr.axis[0], with_conjugate=True
        )
    ...
    def _print_NumpyMatmul(self, expr):
        ...
        if assign:
            ...
            errors.report(
                "The result of a matrix multiplication must be saved into a variable",
                symbol=expr,
                severity="error",
            )
            return ""
        ...
        if dtype.primitive_type is PrimitiveBooleanType():
            errors.report(
                "Boolean matrix multiplication is not implemented",
                symbol=expr,
                severity="error",
            )
        elif len({expr.a.dtype, expr.b.dtype, expr.dtype}) != 1:
            errors.report(
                "Matrix multiplication is only implemented in C when all arguments and results have the same type",
                symbol=expr,
                severity="error",
            )
        if expr.b.rank == 1:
    ...
    def _print_FunctionDef(self, expr):
        ...
        if len(expr.body.get_attribute_nodes(Return)) == 0:
            extra_deallocs = [
                v
                for v in expr.local_vars
                if isinstance(v.class_type, MemoryHandlerType) and v.is_temp
            ]
    ...
    def _print_Return(self, expr):
        ...
        if expr.stmt:
            ...
            last_assign = expr.stmt.get_attribute_nodes(
                (Assign, AliasAssign), excluded_nodes=FunctionCall
            )
            deallocate_nodes = expr.stmt.get_attribute_nodes(
                Deallocate, excluded_nodes=(Assign,)
            )
            ...
            if not last_assign:
                ...
                unneeded_var = (
                    not any(b in vars_in_deallocate_nodes or b.rank for b in variables)
                    and isinstance(last_assign.lhs, Variable)
                    and not last_assign.lhs.is_ndarray
                )
                if unneeded_var:
                    code = code + "".join(
                        self._print(a) for a in expr.stmt.body if a is not last_assign
                    )
    ...
    def _print_Assign(self, expr):
        lhs = expr.lhs
        rhs = expr.rhs
        if isinstance(rhs, (FunctionCall, DictPopitem)) and isinstance(
            rhs.class_type, InhomogeneousTupleType
        ):
        ...
        if isinstance(rhs, NumpyMatmul) or (
            isinstance(rhs, NumpyReduction) and rhs.arg.rank
        ):
    ...
    def _print_AliasAssign(self, expr):
        ...
        if (
            ...
            managed_mem_lst = lhs_var.get_direct_user_nodes(
                lambda u: isinstance(u, ManagedMemory)
            )
            if managed_mem_lst:
                managed_mem = managed_mem_lst[0]
    ...
    def _print_For(self, expr):
        ...
        if isinstance(
            ...
            iterable.variable.class_type,
            (DictType, HomogeneousSetType, HomogeneousListType),
        ):
            ...
            for_code = (
                f"for (c_each ({self._print(counter)}, {c_type}, {iterable_code}))"
            )
            tmp_ref = DottedVariable(
                VoidType(), "ref", memory_handling="alias", lhs=counter
            )
            if isinstance(iterable, DictItems):
                assigns = [
                    Assign(
                        expr.target[0], DottedVariable(VoidType(), "first", lhs=tmp_ref)
                    ),
                    Assign(
                        expr.target[1],
                        DottedVariable(VoidType(), "second", lhs=tmp_ref),
                    ),
                ]
    ...
    def _print_MaxLimit(self, expr):
        ...
        if isinstance(class_type.primitive_type, PrimitiveIntegerType):
            ...
            numpy_type = numpy_precision_map[
                (class_type.primitive_type, class_type.precision)
            ]
            if numpy_type is NumpyFloat32Type():
                ...
                return "LDBL_MAX"
        raise errors.report(PYCCEL_INTERNAL_ERROR, symbol=expr, severity="fatal")
    ...
    def _print_ListPop(self, expr):
        ...
        if expr.index_element:
            self.add_import(
                Import("stc/vec", AsName(VariableTypeAnnotation(class_type), c_type))
            )
            if is_literal_integer(expr.index_element) and int(expr.index_element) < 0:
                idx_code = self._print(
                    PyccelAdd.make_simplified(
                        PythonLen(expr.list_obj), expr.index_element
                    )
                )
            else:
        ...
        if not isinstance(
            expr.class_type, FixedSizeNumericType
        ) and not expr.get_direct_user_nodes(
            lambda u: isinstance(u, UnpackManagedMemory)
        ):
    ...
    def _print_ListReverse(self, expr):
        ...
        list_obj = self._print(ObjectAddress(expr.list_obj))
        self.add_import(
            Import("stc/algorithm", AsName(VariableTypeAnnotation(class_type), c_type))
        )
    ...
    def _print_SetPop(self, expr):
        ...
        self.add_import(
            Import("stc/hset", AsName(VariableTypeAnnotation(dtype), var_type))
        )
    ...
    def _print_SetUnion(self, expr):
        ...
        if not assign_base:
            errors.report(
                "The result of the union call must be saved into a variable",
                severity="error",
                symbol=expr,
            )
        ...
        self.add_import(
            Import("stc/hset", AsName(VariableTypeAnnotation(class_type), var_type))
        )
        set_var = self._print(ObjectAddress(expr.set_obj))
        args = ", ".join(
            [str(len(expr.args)), *(self._print(ObjectAddress(a)) for a in expr.args)]
        )
        # See pyccel/stdlib/STC_Extensions/Set_extensions.h for the definition
    ...
    def _print_SetIntersectionUpdate(self, expr):
        ...
        # See pyccel/stdlib/STC_Extensions/Set_extensions.h for the definition
        return "".join(
            f"{var_type}_intersection_update({set_var}, {self._print(ObjectAddress(a))});\n"
            for a in expr.args
        )
    ...
    def _print_SetDifferenceUpdate(self, expr):
        ...
        # See pyccel/stdlib/STC_Extensions/Set_extensions.h for the definition
        return "".join(
            f"{var_type}_difference_update({set_var}, {self._print(ObjectAddress(a))});\n"
            for a in expr.args
        )
    ...
    def _print_PythonSet(self, expr):
        tmp_var = self.scope.get_temporary_variable(
            expr.class_type, shape=expr.shape, memory_handling="heap"
        )
    ...
    def _print_DictGet(self, expr):
        ...
        raise errors.report(
            "Accessing dictionary using get without default is not supported yet",
            symbol=expr,
            severity="fatal",
        )
