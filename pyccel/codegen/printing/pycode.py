...
import_source_swap = {"omp_lib": "pyccel.stdlib.internal.openmp"}
...
class PythonCodePrinter(CodePrinter):
        ...
        while not isinstance(body, (Assign, ListAppend)):
            if isinstance(body, If):
                ...
                if len(body) > 1:
                    ...
                    if any(
                        not (isinstance(b, Assign) and b.lhs is dummy_var)
                        for b in body[1:]
                    ):
                        raise NotImplementedError(
                            "Pyccel has introduced unnecessary statements which it cannot yet disambiguate in the python printer"
                        )
                body = body[0]
                ...
                raise NotImplementedError(
                    "Type {} not handled in a FunctionalFor".format(type(body))
                )
    ...
    def _get_numpy_name(self, expr):
        ...
        if name == type_name and cls not in (
            PythonBool,
            PythonInt,
            PythonFloat,
            PythonComplex,
    ...
    def _get_type_annotation(self, obj):
        ...
        if isinstance(obj, FunctionDefArgument):
            is_temp_union_name = isinstance(
                obj.annotation, SyntacticTypeAnnotation
            ) and isinstance(obj.annotation.dtype, PyccelSymbol)
            ...
            if isinstance(obj, Variable):
                if obj.is_alias:
                    self.add_import(
                        Import("typing", [AsName(TypingAnnotation, "Annotated")])
                    )
            ...
            args = ", ".join(
                self._get_type_annotation(a).strip("'") for a in obj.arguments
            )
    ...
    def _function_signature(self, func):
        ...
        if interface:
            self.add_import(
                Import("typing", [AsName(FunctionDef("overload", (), ()), "overload")])
            )
            overload = "@overload\n"
        else:
            overload = ""
        ...
        if arguments and arguments[0].is_posonly:
            arg_code.insert(
                next(
                    (i for i, a in enumerate(arguments) if not a.is_posonly),
                    len(arg_code),
                ),
                "/",
            )
        if (
            arguments
            and any(a.is_kwonly for a in arguments)
            and all(not a.is_vararg for a in arguments)
        ):
            arg_code.insert(
                next(
                    (i for i, a in enumerate(arguments) if a.is_kwonly), len(arg_code)
                ),
                "*",
            )
        args = ", ".join(arg_code)
    ...
    def _get_type_var_declarations(self):
        ...
        type_vars_in_scope = {
            n: t
            for n, t in self.scope.symbolic_aliases.items()
            if isinstance(t, TypingTypeVar)
        }
        type_var_constraints = [
            ", ".join(f"'{self._print(ti)}'" for ti in t.type_list)
            for t in type_vars_in_scope.values()
        ]
    ...
    def get_type_checks(self, arg_code, class_type):
        ...
        if isinstance(class_type, NumpyNDArrayType):
            ...
            check_option.append(
                f"{arg_code}.dtype is {dtype}({self._print(class_type.element_type)})"
            )
            check_option.append(f"{arg_code}.ndim == {class_type.rank}")
            if class_type.order:
                check_option.append(
                    f"{arg_code}.flags['{class_type.order}_CONTIGUOUS']"
                )
            ...
            class_type, (HomogeneousListType, HomogeneousSetType, HomogeneousTupleType)
        ):
            check_option = []
            ...
            raise NotImplementedError(
                f"Can't print a Python interface for type {class_type}"
            )
    ...
    def _print_Variable(self, expr):
        if isinstance(expr.class_type, InhomogeneousTupleType):
            elems = ", ".join(
                self._print(self.scope.collect_tuple_element(v)) for v in expr
            )
    ...
    def _print_Interface(self, expr):
        ...
        if len(bodies) == 1:
            ...
            for b, arg_types in bodies.items():
                ...
                import_end = next(
                    i
                    for i, l in enumerate(lines[import_start:], import_start)
                    if not (
                        l.strip().startswith("import ") or l.strip().startswith("from ")
                    )
                )
            ...
            for i, (b, arg_types) in enumerate(bodies_to_print.items()):
                ...
                for a_t in arg_types:
                    checks.append(
                        " and ".join(
                            self.get_type_checks(a, t) for a, t in zip(arg_names, a_t)
                        )
                    )
                if len(checks) > 1:
            ...
            return "\n".join(
                [l for l in (header, docstr, imports_code, code) if l != ""]
            )
    ...
    def _print_FunctionDef(self, expr):
        if expr.is_inline:
            self.add_import(
                Import(
                    "pyccel.decorators",
                    [AsName(FunctionDef("inline", (), ()), "inline")],
                )
            )
        ...
        interfaces = "".join(
            self._print(i) for i in expr.interfaces if not i.is_argument
        )
        functions = [
            f
            for f in expr.functions
            if not any(f in i.functions for i in expr.interfaces)
        ]
        ...
        result_annotation = (
            ("-> '" + self._print(expr.results.annotation) + "'")
        ...
        if expr.stmt:
            to_print = [
                ...
                if not (
                    (
                        isinstance(l, Assign)
                        and isinstance(l.lhs, Variable)
                        and l.lhs in result_vars
                    )
            ...
            if any(
                expr.stmt.body.index(p) < expr.stmt.body.index(a)
                for p in to_print
    ...
    def _print_Program(self, expr):
        ...
        module = modules[0]
        imports = "".join(
            self._print(i) for i in expr.imports if i.source_module is not module
        )
        body = self._print(expr.body)
    ...
    def _print_PythonInt(self, expr):
        arg = self._print(expr.arg)
        if expr.rank:
            name = self._get_numpy_name(
                DtypePrecisionToCastFunction[numpy_process_dtype(expr.dtype)]
            )
    ...
    def _print_NumpyComplex(self, expr):
        ...
        if expr.is_cast:
            ...
            return "{}({}+{}*1j)".format(
                name, self._print(expr.real), self._print(expr.imag)
            )
    ...
    def _print_PythonEnumerate(self, expr):
        if expr.start == 0:
            return "enumerate({elem})".format(elem=self._print(expr.element))
        else:
            return "enumerate({elem},{start})".format(
                elem=self._print(expr.element), start=self._print(expr.start)
            )

    def _print_PythonMap(self, expr):
        return "map({func}, {args})".format(
            func=self._print(expr.func.name), args=self._print(expr.func_args)
        )
    ...
    def _print_PyccelArrayShapeElement(self, expr):
        ...
        if isinstance(
            arg.class_type, (NumpyNDArrayType, HomogeneousTupleType)
        ) or not isinstance(index, LiteralInteger):
            ...
            raise NotImplementedError(
                "The shape access function seems to be poorly defined."
            )
    ...
    def _print_FunctionCall(self, expr):
        ...
        if (
            func.scope.get_python_name(func.name) in ("__init__", "__del__")
            and func.is_imported
    ...
    def _print_Import(self, expr):
        ...
        target = [
            ...
            if not (
                isinstance(t.object, Module)
                or (
                    isinstance(t.object, FunctionDef)
                    and t.object.scope
                    and t.object.scope.get_python_name(t.object.name)
                    in ("__init__", "__del__")
                )
            )
        ]
        ...
        if mod_target:
            if source in pyccel_builtin_import_registry:
                prefix = "".join(
                    f"import {t.name} as {t.local_alias}\n" for t in mod_target
                )
            else:
        ...
        if target:
            if source in import_object_swap:
                target = [
                    AsName(
                        import_object_swap[source].get(i.object, i.object),
                        i.local_alias,
                    )
                    for i in target
                ]
            if source in import_target_swap:
                ...
                target = [
                    AsName(
                        i.object,
                        import_target_swap[source].get(i.local_alias, i.local_alias),
                    )
                    for i in target
                ]
            ...
            if source in pyccel_builtin_import_registry:
                self._aliases.update(
                    (
                        pyccel_builtin_import_registry[source][t.name].cls_name,
                        t.local_alias,
                    )
                    for t in target
                    if not isinstance(t.object, VariableTypeAnnotation)
                    and t.name != t.local_alias
                )

            if init_func:
                self._ignore_funcs.append(init_func)
                if init_func.name in self.scope.imports["functions"]:
                    self._ignore_funcs.append(
                        self.scope.imports["functions"][init_func.name]
                    )
            if free_func:
                self._ignore_funcs.append(free_func)

            target = [
                self._print(t) for t in target if t.object not in (init_func, free_func)
            ]
            target = ", ".join(target)
    ...
    def _print_FunctionalFor(self, expr):
        ...
        for_loops = " ".join(
            f'for {self._print(idx)} in {self._print(iters)}{" if " + self._print(condition.blocks[0].condition) if condition else ""}'
    ...
    def _print_While(self, expr):
        ...
        self.exit_scope()
        return "while {cond}:\n{body}".format(cond=cond, body=body)
    ...
    def _print_Assign(self, expr):
        ...
        if (
            ...
            and (rhs.class_type, InhomogeneousTupleType)
            and isinstance(lhs, PythonTuple)
        ):
        ...
        if isinstance(lhs, IndexedElement) and isinstance(
            lhs.base.class_type, HomogeneousTupleType
        ):
    ...
    def _print_NumpyMatmul(self, expr):
        name = self._aliases.get(type(expr), expr.name)
        return "{0}({1}, {2})".format(name, self._print(expr.a), self._print(expr.b))
    ...
    def _print_NumpyCross(self, expr):
        ...
        if axisa == axisb == axisc:
            ...
            return (
                f"{c} = {name}({a}, {b}, axisa={axisa}, axisb={axisb}, axisc={axisc})\n"
            )
    ...
    def _print_Slice(self, expr):
        ...
        return "{start}:{stop}:{step}".format(start=start, stop=stop, step=step)
    ...
    def _print_SetMethod(self, expr):
        ...
        args = (
            ""
            if len(expr.args) == 0 or expr.args[-1] is None
    ...
    def _print_Module(self, expr):
        ...
        for i in expr.imports:
            ...
            if source in pyccel_builtin_import_registry:
                self._aliases.update(
                    ...
                    for t in i.target
                    if not isinstance(t.object, (Module, VariableTypeAnnotation))
                    and t.name != t.local_alias
                )
        ...
        funcs = [
            ...
            for f in expr.funcs
            if not (
                any(f in i.functions for i in expr.interfaces)
                or f is expr.init_func
                or f is expr.free_func
            )
        ]
    ...
    def _print_ModuleHeader(self, expr):
        ...
        init_func = mod.init_func
        var_decl = "".join(
            f"{mod.scope.get_python_name(v.name)} : {self._get_type_annotation(v)}\n"
            for v in variables
            if not v.is_temp
        )
        funcs = "".join(f"{self._function_signature(f)}\n" for f in mod.funcs)
        funcs += "".join(
            f"{self._function_signature(f)}\n"
            for i in mod.interfaces
            if not i.is_inline
            for f in i.functions
        )
        funcs += "".join(
            f"{self._function_signature(i.functions[0])}\n"
            for i in mod.interfaces
            if i.is_inline
        )
        classes = ""
        for classDef in mod.classes:
            ...
            class_body = (
                "\n".join(
                    f"{classDef.scope.get_python_name(v.name)} : {self._get_type_annotation(v)}"
        ...
        return "\n".join(
            section
            for section in (imports, type_var_declarations, var_decl, classes, funcs)
            if section
        )
    ...
    def _print_ConstructorCall(self, expr):
        ...
        try:
            cls_name = self.scope.get_import_alias(
                cls_variable.class_type, "cls_constructs"
            )
        except RuntimeError:
