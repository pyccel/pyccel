...
This decorator is not designed to be used in a `.py` file. Rather it should be used in `.pyi` files. These files are generated by Pyccel to speed up multi-file compilation. They can also be used with `pyccel-wrap` to expose existing Fortran or C code to Python. The decorator indicate the low-level name of the function being wrapped. See [Interfacing Python code with low-level code](./low-level-interface.md) for more information.
