...
c : set[float] = {}
```
So far sets can be declared as local variables or as constant arguments. An argument can be marked as constant using a string annotation or (in a module) using the `Final` qualifier:
```python
