...
In addition, for each tool used that requires additional compilation information (e.g. `mpi`/`openmp`/`openacc`/`python`), the JSON file must define the following:
...
Python must additionally specify `shared_suffix`.
