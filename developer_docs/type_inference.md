...
In order to handle pointers, these types implement a `is_alias` method which checks if a Variable of this type would describe an alias to another Variable or not. Where appropriate the function `get_alias_equivalent` is also provided. This function gets the type of an alias to the basic type (e.g `int*` from `int`).
