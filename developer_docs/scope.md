...
In the translations generated by Pyccel we try to stay as close as possible to the names in the original Python code. However this is not always possible as different languages have different restrictions on names. For example the following code cannot be translated to Fortran as is:
