...
if __name__ == "__main__":
            ...
            while i < n and lines[i].startswith(" - "):
                if lines[i].startswith(" - No module docstring"):
                    ...
                    if not should_ignore(objname):
                        ...
                        mod_dict.setdefault(obj_name_parts[0], []).append(
                            obj_name_parts[1:]
                        )
                i += 1

    added_mod = [
        mod for mod in results["compare_no_mod"] if mod not in results["base_no_mod"]
    ]
    added_obj = {
        (mod, cls): methods
        for mod, obj in results["compare_no_obj"].items()
        for cls, methods in obj.items()
        if any(
            m not in results["base_no_obj"].get(mod, {}).get(cls, []) for m in methods
        )
    }
    ...
    if len(added_mod) > 0 or len(added_obj) > 0:
        ...
        if len(added_mod) > 0:
            print_to_string(
                "### This pull request added these modules without docstrings:",
                text=summary,
            )
        ...
        if len(added_obj) > 0:
            print_to_string(
                "### This pull request added these objects without docstrings:",
                text=summary,
            )
            idx = 0
            for (mod, cls), objects in added_obj.items():
                ...
                for obj in objects:
                    ...
                    if obj in results["base_no_obj"].get(mod, {}).get(cls, []):
                        ...
                        level = "failure"
                        print_to_string(
                            f"{idx + 1}.  {mod}.{cls}.{obj_name}", text=summary
                        )
                        idx += 1
                    try:
                        file, start, end = get_code_file_and_lines(
                            f"{cls}.{obj_name}", base_folder, mod
                        )
            ...
            print_to_string(text=summary)
        summary_text = "\n".join(summary)
        messages = {"summary": summary_text, "annotations": annotations}
