...
def get_pylint_results(filename):
    ...
    while not all(c == "-" for c in line):
        if not line.startswith("***"):
            ...
            pylint_results.setdefault(file, []).append(
                PylintMessage(file, line, start, message.strip())
            )
        idx += 1
        line = pylint_output[idx]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Parse pylint output and format the output for neat bot results."
    )
    parser.add_argument(
        "compare_pylint",
        type=str,
