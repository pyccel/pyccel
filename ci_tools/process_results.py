...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process the output of numpydoc validator"
    )
    parser.add_argument(
        "report",
        metavar="report",
    ...
    parser.add_argument(
        "summary", metavar="summary", type=str, help="Github step summary"
    )
            ...
            if level == "failure":
                ...
                if code != "GL08":
                    ...
                    if code.startswith("PR"):
                        ...
                        if not changed:
                            try:
                                idx = next(
                                    i
                                    for i, (sec, l) in enumerate(sections)
                                    if sec == "Parameters"
                                )
                                if idx < nsections - 1:
                    ...
                    if code.startswith("RT") and code != "RT01":
                        idx = next(
                            i for i, (sec, l) in enumerate(sections) if sec == "Returns"
                        )
                        if idx < nsections - 1:
                            end = start + sections[idx + 1][1] - 1
                        start += sections[idx][1]
    ...
    if len(parsing_errors) > 0:
        ...
        print_to_string(
            "".join(f"{add_warn}" for add_warn in parsing_errors), text=summary
        )
