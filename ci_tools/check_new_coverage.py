...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Check that all new lines in the python files in the pyccel/ code folder are used in the tests"
    )
    parser.add_argument(
        "diffFile",
        metavar="diffFile",
    ...
    old_comments, new_comments, fixed_comments = cov.get_json_summary(
        new_untested, file_contents, commented_lines, diff
    )

    success = cov.evaluate_success(old_comments, new_comments, commented_lines)

    cov.print_markdown_summary(
        old_comments + new_comments, os.environ["HEAD_SHA"], args.output, bot.repo
    )
