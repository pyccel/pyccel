...
def extract_dict_elements(input_dict):
    ...
    for values in input_dict.items():
        ...
        if value_list:
            ...
            output_dict["summary"] += "\n\n".join(
                f"-  {value['message']}" for value in value_list
            )
            output_dict["summary"] += "\n\n"
            output_dict["annotations"].extend(
                value["annotations"] for value in value_list
            )
    return output_dict


def fill_dictionary(
    title, message, file, start_line, end_line, annotation_level, annotation_msg
):
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Check that Pyccel coding conventions are respected."
    )
    ...
    ast_modules = [
        ".".join(f.relative_to(ast_folder).parts)[:-3]
        for f in ast_folder.rglob("*.py")
        if f.parts[-1] != "__init__.py"
    ]
    ...
    for mod_name in ast_modules:
        ...
        if all_attr:
            ...
            if sorted_all != list(all_attr):
                ...
                all_keys = [
                    li.strip(" ,'\"")
                    for l in all_code.strip()
                    .rstrip(")")
                    .removeprefix("__all__")
                    .strip()
                    .lstrip("= (")
                    .split(",")
                    for li in l.split("\n")
                ...
                if len(set(groups)) < len(groups):
                    error_collection["badly_grouped_all"].append(
                        fill_dictionary(
                            "`__all__` split into multiple groups with the same name",
                            f"pyccel.ast.{mod_name}",
                            inspect.getfile(mod),
                            start_line,
                            end_line,
                            "failure",
                            f"Fix the groups in the __all__ attribute of `{mod_name}`",
                        )
                    )
                if any(len(g) == 0 for g in groups):
                    error_collection["bad_all_group"].append(
                        fill_dictionary(
                            "`__all__` is split into unlabelled groups",
                            f"pyccel.ast.{mod_name}",
                            inspect.getfile(mod),
                            start_line,
                            end_line,
                ...
                for n, g in groups.items():
                    ...
                    if g != o_g:
                        ...
                        if n != "start":
                            name += f"[{n}]"
                        error_collection["non_alphabetical_all"].append(
                            fill_dictionary(
                                ...
                                end_line,
                                "failure",
                                f"Sort the __all__ attribute of `{mod_name}`",
                            )
                        )
        else:
            error_collection["missing_all"].append(
                fill_dictionary(
                    "Missing `__all__`",
                    f"pyccel.ast.{mod_name}",
                    inspect.getfile(mod),
                    ...
                    1,
                    "failure",
                    f"Missing __all__ attribute in: `{mod_name}`",
                )
            )
        ...
        for cls_name, cls_obj in classes:
            ...
            if "__slots__" not in cls_obj.__dict__:
                ...
                error_collection["missing_slots"].append(
                    fill_dictionary(
                        "Classes with no `__slots__`",
                        f"pyccel.ast.{mod_name}.{cls_name}",
                        inspect.getfile(mod),
                        sourceline,
                        sourceline,
                        "failure",
                        f"`{mod_name}.{cls_name}` class with no `__slots__`",
                    )
                )
            else:
                ...
                for c in super_classes:
                    if "__slots__" not in c.__dict__:
                        ...
                        sourceline = inspect.getsourcelines(cls_obj)[1]
                        error_collection["overridden_slots"].append(
                            fill_dictionary(
                                "Overwritten slot values",
                                f"pyccel.ast.{mod_name}.{cls_name}",
                                inspect.getfile(mod),
                                sourceline,
                                sourceline,
                                "failure",
                                f"Slot values are overwritten between `{mod_name}.{cls_name}` and `{c.__name__}`",
                            )
                        )

            if PyccelAstNode in super_classes:
                if cls_obj not in (TypedAstNode, ScopedAstNode) and not isinstance(
                    cls_obj._attribute_nodes, tuple
                ):  # pylint: disable=W0212
                    sourceline = inspect.getsourcelines(cls_obj)[1]
                    error_collection["missing_attribute_nodes"].append(
                        fill_dictionary(
                            "Classes with no `_attribute_nodes`",
                            f"pyccel.ast.{mod_name}.{cls_name}",
                            inspect.getfile(mod),
                            sourceline,
                            sourceline,
                            "failure",
                            f"Missing attribute nodes in : `{mod_name}.{cls_name}`",
                        )
                    )

            if all_attr and cls_name not in all_attr:
                sourceline = inspect.getsourcelines(cls_obj)[1]
                error_collection["missing_from_all"].append(
                    fill_dictionary(
                        ...
                        sourceline,
                        "failure",
                        f"`{mod_name}.{cls_name}` is missing from `__all__`",
                    )
                )
    ...
    failure = (
        bool(
            error_collection["missing_all"]
        )  # bool(error_collection['non_alphabetical_all']) or
        or bool(error_collection["missing_slots"])
        or bool(error_collection["missing_attribute_nodes"])
        or bool(error_collection["overridden_slots"])
        or bool(error_collection["missing_from_all"])
    )
