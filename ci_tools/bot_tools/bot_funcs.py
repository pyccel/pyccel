...
class Bot:
    ...
    def get_bot_review_comments(self):
        ...
        bot_grouped_comments = [
            c
            for c in grouped_comments.values()
            if c[0]["user"].get("type", "user") == "Bot"
        ]

        relevant_comments = [
            c for c in bot_grouped_comments if c[0]["position"] is not None
        ]
        discarded_comments = [
            c for c in bot_grouped_comments if c[0]["position"] is None
        ]
    ...
    def accept_coverage_fix(self, comment_thread):
        ...
        print("id: ", comment_id)
        reply = self._GAI.create_comment(self._pr_id, message, reply_to=comment_id)
        print(reply.text)
