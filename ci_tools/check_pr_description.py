...
if first_line.strip() == original_body[0].strip():
    print(
        "ERROR: The PR description still contains the boilerplate placeholder. "
...
if len(pr_description.split()) <= 2:
    print(
        "ERROR: The PR description is too short. "
...
if "- [ ]" in body:
    print(
        "ERROR: Not all checklist items have been ticked. "
        "Please complete the PR checklist before marking the PR as ready."
    )
    sys.exit(1)

if body.count("- [x]") != sum(
    1 if line.startswith("- [ ]") else 0 for line in original_body
):
    print(
        "ERROR: Missing checklist items. "
